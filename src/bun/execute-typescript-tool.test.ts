  ...
  it("lists workflow authoring assets with spec-aligned metadata and filters", async () => {
    ...
    writeWorkspaceFile(
      ...
      [
        ...
        " * @svvySummary Reusable workflow factory for implement, review, and quality stages.",
        " */",
    ...
    const result = await tool.execute("tool-call-workflow-assets", {
      typescriptCode: [
        ...
        '  scope: "saved",',
        "});",
        "const components = await api.workflow.listAssets({",
        '  kind: "component",',
        '  scope: "both",',
        "});",
        ...
        "  promptPaths: prompts.map((asset) => asset.path),",
        "  componentPaths: components.map((asset) => asset.path),",
    ...
    const [parentCommand, ...childCommands] = snapshot.commands;
    expect(parentCommand?.summary).toContain("Discovered 7 workflow assets");
    expect(childCommands.map((command) => command.toolName)).toEqual([
      "workflow.listAssets",
      "workflow.listAssets",
      "workflow.listAssets",
