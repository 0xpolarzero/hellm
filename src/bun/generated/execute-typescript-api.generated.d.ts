...
interface SvvyApi {
  ...
  repo: {
     ...
     */
    readFiles(input: { paths: string[] }): Promise<{
      files: RepoTextFile[];
    }>;
    /**
     * Read and JSON-parse one workspace file.
     */
    readJson<T>(input: { path: string }): Promise<{
      path: string;
      value: T;
    }>;
    /**
     ...
     */
    unlink(input: { path: string }): Promise<{
      path: string;
      deleted: boolean;
    }>;
    /**
  ...
  git: {
     ...
     */
    mergeBase(input: { baseRef: string; headRef: string }): Promise<{
      sha?: string;
    }>;
    fetch(input?: {
      remote?: string;
