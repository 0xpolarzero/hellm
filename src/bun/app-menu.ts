...
export type AppMenuRendererCommand =
  | "command-palette.open"
  | "quick-open.open"
  | "settings.open"
  | Exclude<AppMenuAction, "commandPalette.open" | "quickOpen.open">;

export type AppMenuNativeAction = AppMenuAction | "settings.open";

export type AppMenuActionRoute = {
  readonly kind: "renderer-command";
  readonly command: AppMenuRendererCommand;
};
...
export function routeAppMenuAction(action: unknown): AppMenuActionRoute | null {
  ...
  return { kind: "renderer-command", command: action };
}
