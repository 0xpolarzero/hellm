...
export async function discoverModels(
	...
	if (type === "ollama") {
		const data = await fetchJson<{ models?: Array<{ name?: string }> }>(`${baseUrl.replace(/\/$/, "")}/api/tags`, apiKey);
		return (data.models || [])
	...
	const data = await fetchJson<{
		data?: Array<{ id?: string; context_length?: number; max_model_len?: number; max_tokens?: number }>;
