...
export type ArtifactKind = "html" | "svg" | "markdown" | "image" | "pdf" | "excel" | "docx" | "text" | "generic";
...
export const artifactsParamsSchema = Type.Object({
	...
	filename: Type.String({ description: "Filename including extension, for example index.html or notes.md." }),
	content: Type.Optional(Type.String({ description: "File content for create or rewrite operations." })),
	old_str: Type.Optional(Type.String({ description: "Existing string to replace for update operations." })),
...
export function getArtifactCommandCopy(command: ArtifactCommand): { inProgress: string; complete: string } {
...
export class ArtifactsController {
	...
	async reconstructFromMessages(messages: AgentMessage[]): Promise<void> {
		...
		this.activeFilename = this.artifacts.size > 0 ? Array.from(this.artifacts.keys())[0] ?? null : null;
		this.emit();
	}
	...
	private async executeCommand(params: ArtifactsParams, options: ExecuteOptions = {}): Promise<string> {
		switch (params.command) {
			case "create":
	...
	private async updateArtifact(params: ArtifactsParams, options: ExecuteOptions): Promise<string> {
		...
		if (!artifact) {
			...
			if (available.length === 0) return `Error: File ${params.filename} not found. No files have been created yet.`;
	...
	private async refreshHtmlArtifacts(excludeFilename?: string): Promise<void> {
		...
		const htmlArtifacts = Array.from(this.artifacts.values()).filter(
			(artifact) => getArtifactKind(artifact.filename) === "html" && artifact.filename !== excludeFilename,
		);
	...
	private buildHtmlDocument(content: string, token?: string): string {
		const snapshot = JSON.stringify(
			Object.fromEntries(Array.from(this.artifacts.entries()).map(([filename, artifact]) => [filename, artifact.content])),
		);
		const runtimeScript = `
			<script>
				(() => {
...
export function buildArtifactResultsMap(messages: AgentMessage[]): Map<string, string> {
	...
	for (const message of messages) {
		...
		results.set(message.toolCallId, getTextResult(message.content as Array<{ type: string }> | undefined));
	}
	return results;
}
