        ...
        <section>
          ...
          <div className="border border-border rounded divide-y divide-border overflow-hidden">
            {providers.map((p) => (
              <div
                key={p.name}
                className="flex items-center gap-3 px-4 py-3 bg-card hover:bg-card/80 transition-colors"
              >
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2">
                    ...
                    {p.status === "connected" && (
                      <span className="font-mono text-[9px] text-emerald-400 border border-emerald-500/20 bg-emerald-500/8 rounded px-1">
                        connected
                      </span>
                    )}
                  </div>
                  ...
                  {p.status === "connected" ? (
                    <>
                      <button
                        ...
                        data-testid={`btn-edit-${p.name}`}
                      >
                        Edit
                      </button>
                      <button
                        className="text-[11px] text-red-400 hover:text-red-300 border border-red-500/20 rounded px-2 py-1 transition-colors"
                        data-testid={`btn-revoke-${p.name}`}
                      >
                        Revoke
                      </button>
                    </>
                    ...
                    <button
                      className="text-[11px] text-foreground/80 hover:text-foreground border border-border rounded px-2.5 py-1 transition-colors flex items-center gap-1"
                      data-testid={`btn-connect-${p.name}`}
                    >
                    ...
                    <button
                      className="text-[11px] text-foreground/80 hover:text-foreground border border-border rounded px-2.5 py-1 transition-colors flex items-center gap-1"
                      data-testid={`btn-connect-key-${p.name}`}
                    >
            ...
            {oauthConnections.map((c) => (
              ...
              >
                ...
                <div className="flex-1 min-w-0">
                  ...
                  {c.user && <span className="text-[11px] text-muted-foreground">{c.user}</span>}
                </div>
                <div className="flex-shrink-0">
                  {c.status === "connected" ? (
                    <button
                      className="text-[11px] text-red-400 hover:text-red-300 border border-red-500/20 rounded px-2 py-1 transition-colors"
                      data-testid={`btn-disconnect-${c.name}`}
                    >
                      Disconnect
                    </button>
                  ) : (
                    <button
                      className="text-[11px] text-foreground/80 hover:text-foreground border border-border rounded px-2.5 py-1 transition-colors flex items-center gap-1"
                      data-testid={`btn-connect-oauth-${c.name}`}
                    >
            ...
            {envCreds.map((c) => (
              <div key={c.name} className="flex items-center justify-between px-4 py-2.5 bg-card">
                ...
                {c.found ? (
                  ...
                  <span className="font-mono text-[9px] text-muted-foreground border border-border rounded px-1.5 py-0.5">
                    not found
                  </span>
                )}
              </div>
