...
import {
  Bot, Terminal, CheckCircle2, Clock, RefreshCw, Flag, Shield, Zap
} from "lucide-react";
...
const nodeTypeConfig: Record<NodeType, {
  ...
  "script": { icon: Terminal, border: "border-purple-500/40", bg: "bg-purple-500/8", label: "script" },
  "verification": { icon: CheckCircle2, border: "border-emerald-500/40", bg: "bg-emerald-500/8", label: "verify" },
            ...
            {edges.map((edge, i) => {
              ...
              return (
                <path
                  ...
                  fill="none"
                  stroke={isDone ? "rgba(34,197,94,0.4)" : isActive ? "rgba(249,115,22,0.5)" : "rgba(255,255,255,0.08)"}
                  strokeWidth="1.5"
            ...
            return (
              ...
              >
                ...
                <div className="min-w-0">
                  <div className="text-[10px] font-mono text-foreground/90 truncate leading-tight">{node.label}</div>
                  <div className={cn(
                ...
                {node.elapsed && (
                  <span className="font-mono text-[8px] text-muted-foreground ml-auto flex-shrink-0">{node.elapsed}</span>
                )}
              </button>
            );
          })}
      ...
      {selectedNode && (
        <div className="border-t border-border bg-card flex-shrink-0 px-4 py-3 space-y-2.5 max-h-56 overflow-y-auto scrollbar-thin">
          ...
          {selectedNode.objective && (
            <p className="text-[12px] text-foreground/80 leading-relaxed">{selectedNode.objective}</p>
          )}

          {selectedNode.latestOutput && (
            <div>
              <div className="text-[9px] font-mono text-muted-foreground uppercase tracking-wider mb-1">Latest output</div>
          ...
          <div className="flex items-center gap-4 flex-wrap">
            {selectedNode.elapsed && (
              <div className="flex items-center gap-1">
                ...
                <span className="font-mono text-[10px] text-foreground/70">{selectedNode.elapsed}</span>
              </div>
            )}
            {selectedNode.model && (
              <div className="flex items-center gap-1">
            ...
            {selectedNode.worktree && (
              <div className="flex items-center gap-1">
                ...
                <span className="font-mono text-[10px] text-foreground/70">{selectedNode.worktree}</span>
              </div>
            )}
          </div>
