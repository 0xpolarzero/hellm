...
export function EpisodeCard({ episode, className }: EpisodeCardProps) {
  return (
    <div
      className={cn(
        "border border-border rounded bg-card px-3 py-2.5",
        className
      )}
      ...
      <div className="flex items-start gap-2 mb-2">
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-start justify-between gap-2">
            <h4 className="text-[12px] font-medium text-foreground leading-tight">{episode.title}</h4>
          ...
          <p className="text-[11px] text-muted-foreground mt-1 leading-relaxed line-clamp-2">{episode.summary}</p>
        </div>
      </div>
