import { CheckCircle2, AlertTriangle, Lightbulb, FileText, GitBranch } from "lucide-react";
      ...
      <div className="flex-1 overflow-hidden">
        {tab === "summary" && (
          <div className="h-full overflow-y-auto scrollbar-thin px-3 py-3 space-y-4">
            <div>
              <h4 className="text-[10px] font-mono text-muted-foreground uppercase tracking-wider mb-1.5">
                Objective
              </h4>
        ...
        {tab === "diff" && <DiffViewer filename="src/middleware/auth.ts" />}

        {tab === "subagents" && (
          <div className="h-full overflow-y-auto scrollbar-thin px-3 py-3 space-y-1.5">
            {threadAgents.map((a) => (
              <SubagentCard key={a.id} agent={a} expandable={true} />
            ))}
          </div>
        )}
      </div>
