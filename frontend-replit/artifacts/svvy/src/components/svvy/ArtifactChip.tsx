...
const typeConfig: Record<ArtifactType, { icon: React.ElementType; color: string; label: string }> =
  {
    ...
    log: {
      icon: FileText,
      color: "text-slate-400 border-slate-500/20 bg-slate-500/8",
      label: "log",
    },
    screenshot: {
      icon: Image,
    ...
    report: {
      icon: BarChart2,
      color: "text-cyan-400 border-cyan-500/20 bg-cyan-500/8",
      label: "report",
    },
    html: {
      icon: FileCode,
...
export function ArtifactChip({ name, type, artifact, className, onClick }: ArtifactChipProps) {
  ...
  const handleClick = () => {
    if (onClick) {
      ...
      openPane(
        paneType,
        artifact || { id: name, name, type, size: "—", age: "—", session: "—" },
        name,
      );
    }
  };
