...
export function ContextBudgetBar({ percent, showLabel = false, width = "w-28", className }: ContextBudgetBarProps) {
  const color =
  ...
  return (
    <div className={cn("flex items-center gap-2", className)} data-testid="context-budget-bar">
      ...
      {showLabel && (
        <span className={cn("font-mono text-[10px] tabular-nums", labelColor)}>
          {percent}%
        </span>
      )}
    </div>
