...
import { X, GitBranch, FileText, CheckCircle2, AlertTriangle, Lightbulb, FolderTree, PanelRight } from "lucide-react";
import { motion, AnimatePresence } from "framer-motion";
import { cn } from "@/lib/utils";
import { StatusBadge } from "./StatusBadge";
import { ModelBadge } from "./ModelBadge";
import { ArtifactChip } from "./ArtifactChip";
import { mockArtifacts, mockEpisodes, mockPaneSurfaces, mockThreads, mockVerification } from "@/data/mock";
        ...
        >
          ...
          <div className="flex items-center justify-between px-3 py-2.5 border-b border-border flex-shrink-0">
            <div className="flex items-center gap-1">
              {(["thread", "episode", "artifact", "workflow", "verification"] as InspectorTab[]).map(tab => (
                <button
                  key={tab}
          ...
          <div className="flex-1 overflow-y-auto scrollbar-thin px-3 py-3 space-y-4">
            {activeTab === "thread" && (
              <ThreadInspector thread={thread} />
            )}
            {activeTab === "episode" && (
              <EpisodeInspector episode={episode} />
            )}
            {activeTab === "artifact" && (
...
function InspectorSection({ title, children }: { title: string; children: React.ReactNode }) {
  return (
    <div>
      <h4 className="text-[10px] font-mono font-semibold text-muted-foreground uppercase tracking-wider mb-1.5">{title}</h4>
      {children}
    </div>
  );
}
    ...
    <>
      ...
      <InspectorSection title="Verification">
        <div className={cn(
          ...
          <CheckCircle2 className={cn("w-3.5 h-3.5", episode.verified ? "text-emerald-500" : "text-red-500")} />
          <span className={cn("text-[12px]", episode.verified ? "text-emerald-400" : "text-red-400")}>
    ...
    <>
      <div>
        ...
        <div className="space-y-1">
          <div className="flex items-center justify-between">
            ...
            <span className="text-[11px] text-foreground/70 truncate max-w-40">{artifact.session}</span>
          </div>
          <div className="flex items-center justify-between">
      ...
      <InspectorSection title="Preview">
        <div className="rounded border border-border bg-muted/30 p-2 text-[10px] font-mono text-muted-foreground h-32 overflow-hidden">
          ...
          <div className="diff-context"> import {"{"} Request, Response {"}"} from 'express';</div>
          <div className="diff-remove">-import {"{"} verifyToken {"}"} from '../utils/jwt';</div>
          <div className="diff-add">+import {"{"} verifyToken, extractOAuthToken {"}"} from '../utils/jwt';</div>
          <div className="diff-add">+import {"{"} OAuthProvider {"}"} from '../types/oauth';</div>
      ...
      <InspectorSection title="Pane surfaces">
        <div className="space-y-1">
          {mockPaneSurfaces.map(surface => (
            <div key={surface.id} className="flex items-center justify-between rounded border border-border px-2 py-1.5">
              ...
              <span className="font-mono text-[9px] text-muted-foreground">{surface.positions.join(" ")}</span>
            </div>
          ))}
        </div>
      </InspectorSection>
  ...
  return (
    <>
      <div>
        ...
        <p className="text-[12px] text-muted-foreground leading-relaxed">{mockVerification.summary}</p>
      </div>
      ...
      <InspectorSection title="Checks">
        <div className="space-y-1">
          ...
          <div className="flex items-center justify-between">
            ...
            <span className="font-mono text-[10px] text-emerald-400">{mockVerification.testsPassed}/{mockVerification.testsTotal}</span>
          </div>
          <div className="flex items-center justify-between">
      ...
      <InspectorSection title="Artifacts">
        <div className="space-y-1">
          {mockVerification.artifacts?.map(artifact => (
            <div key={artifact.id} className="flex items-center justify-between rounded border border-border px-2 py-1.5">
              <div className="flex items-center gap-1.5 min-w-0">
                <FileText className="w-2.5 h-2.5 text-muted-foreground flex-shrink-0" />
      ...
      <InspectorSection title="Projection">
        <div className="rounded border border-border px-2.5 py-2 bg-muted/20">
          ...
          <p className="text-[11px] text-muted-foreground leading-relaxed">
            Verification is linked to the session timeline, inspector, and artifact browser instead of relying on transcript parsing.
          </p>
        </div>
      </InspectorSection>
    </>
