...
test("renders seeded transcript timestamps and tool error states", async () => {
	await launchEdgeCaseApp(async ({ page }) => {
		...
		expect(await firstAssistant.locator(".tool-card.error .tool-status").textContent()).toBe("error");
...
test("renders a distinct reasoning fallback when the seeded thinking text is empty", async () => {
	await launchEdgeCaseApp(async ({ page }) => {
		await waitForExactText(page.locator(".workspace-main-title"), SESSION_TITLE);
