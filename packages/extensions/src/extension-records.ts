...
export type SvvyActorKind = ActorKind;
...
export interface ExtensionExternalInstructionSource {
  id: string;
  kind: "AGENTS.md" | "CLAUDE.md";
  title: string;
  path: string;
  enabled: boolean;
  actors: readonly SvvyActorKind[];
  readStatus: {
...
export const BUILTIN_EXTENSIONS = [
  ...
  },
  {
    ...
    description: "Typed composition snippets with actor-local generated runtime facades.",
    instructionSourceFiles: [],
...
export function workflowTaskReferenceableBuiltinExtensionIds(): string[] {
  return BUILTIN_EXTENSIONS.filter(
    (extension) => getDefaultUsageState(extension.id, "workflow-task") !== "unavailable",
  )
...
export function externalInstructionExtensionId(source: ExtensionExternalInstructionSource): string {
...
function externalInstructionRecords(
  sources: readonly ExtensionExternalInstructionSource[],
  actor?: SvvyActorKind,
): { loaded: VisibleLoadedExtensionRecord[]; available: VisibleAvailableExtensionRecord[] } {
