...
export interface StructuredCommandRollup {
  ...
  stdin: StructuredCommandStdinState;
...
export interface StructuredCommandStdinEvent {
  eventId: string;
  at: string;
  text: string;
...
export interface StructuredCommandStdinState {
  mode: "none" | "continuable";
  canAttemptWrite: boolean;
  acceptedWrites: StructuredCommandStdinEvent[];
}
...
export function buildStructuredArtifactLink(
  ...
  return {
    ...
    ...(artifact.missingFile === true ? { missingFile: true } : {}),
  };
}
...
function buildCommandStdinEvents(
  session: Pick<StructuredSessionSnapshot, "events">,
  commandId: string,
  ...
  return session.events
    .flatMap((event) => {
      if (
        event.kind !== "command.stdin" ||
        event.subject.kind !== "command" ||
        event.subject.id !== commandId
      ) {
        return [];
      }
      const stdinEvent = parseCommandStdinEvent(event);
      return stdinEvent ? [stdinEvent] : [];
    })
    .toSorted(
...
function parseCommandStdinEvent(
  event: StructuredLifecycleEventRecord,
): StructuredCommandStdinEvent | null {
  const data = event.data;
  if (!data || typeof data !== "object") {
    return null;
  }
  const text = (data as { text?: unknown }).text;
  const acceptedBytes = (data as { acceptedBytes?: unknown }).acceptedBytes;
  if (
    typeof text !== "string" ||
...
function isCommandStdinContinuable(command: StructuredCommandRecord): boolean {
  return command.toolName === "exec_command";
}

function isCommandTerminal(command: StructuredCommandRecord): boolean {
  return (
    command.status === "succeeded" || command.status === "failed" || command.status === "cancelled"
  );
}

function buildCommandStdinState(
  command: StructuredCommandRecord,
  session: Pick<StructuredSessionSnapshot, "events"> | null,
): StructuredCommandStdinState {
  const acceptedWrites = session ? buildCommandStdinEvents(session, command.id) : [];
  const mode =
    isCommandStdinContinuable(command) || acceptedWrites.length > 0 ? "continuable" : "none";
  return {
    mode,
    canAttemptWrite: mode === "continuable" && !isCommandTerminal(command),
    acceptedWrites,
  };
}
...
function buildCommandInspectorChild(
  ...
  return {
    ...
    stdin: buildCommandStdinState(command, session),
    ...
    .map((command) => {
      ...
      return {
        ...
        outputEvents: session.events
          ...
          : [],
        stdin: buildCommandStdinState(command, session.events ? { events: session.events } : null),
...
export function buildStructuredSessionSummaryProjection(
  ...
  const projection: StructuredSessionSummaryProjection = {
  ...
  if (view.latestEpisodePreview !== undefined) {
    projection.latestEpisodePreview = view.latestEpisodePreview;
  }
  if (view.latestWorkflowRunSummary !== undefined) {
    projection.latestWorkflowRunSummary = view.latestWorkflowRunSummary;
  }
  return projection;
}
...
export function buildStructuredCommandInspector(
  ...
  return {
    ...
    stdin: buildCommandStdinState(parentCommand, session),
...
export interface StructuredThreadReadModelWait {
  kind: "user" | "external";
  reason: string;
  resumeWhen: string;
}

export interface StructuredThreadReadModelEpisodeSummary {
  id: string;
  title: string;
  summary: string;
  createdAt: string;
...
export interface StructuredThreadCompactRow {
  threadId: string;
  ...
  wait: StructuredThreadReadModelWait | null;
  latestEpisode: StructuredThreadReadModelEpisodeSummary | null;
}

export interface StructuredThreadPendingReportRequest {
...
export interface StructuredThreadListReadModel {
  threads: StructuredThreadCompactRow[];
}
...
export interface StructuredThreadGroupReadModel {
  threadGroupId: string;
  currentThreadId: string;
  threads: StructuredThreadCompactRow[];
}

export interface BuildStructuredThreadListReadModelInput {
  status?: readonly StructuredThreadStatus[] | null;
...
export function getStructuredThread(
  session: StructuredSessionSnapshot,
  threadId: string,
): StructuredThreadRecord | null {
  return session.threads.find((thread) => thread.id === threadId) ?? null;
}

export function hasStructuredThreadGroup(
  session: StructuredSessionSnapshot,
  threadGroupId: string,
): boolean {
  return session.threads.some((thread) => thread.threadGroupId === threadGroupId);
}

export function buildStructuredThreadCompactRow(
  session: StructuredSessionSnapshot,
  thread: StructuredThreadRecord,
): StructuredThreadCompactRow {
  return {
    threadId: thread.id as StructuredThreadCompactRow["threadId"],
    threadGroupId: thread.threadGroupId as StructuredThreadCompactRow["threadGroupId"],
    workspaceSessionId: thread.sessionId as StructuredThreadCompactRow["workspaceSessionId"],
    surfacePiSessionId:
      thread.surfacePiSessionId as StructuredThreadCompactRow["surfacePiSessionId"],
    title: thread.title,
    objective: thread.objective,
    objectiveState: thread.objectiveState,
    status: thread.status,
    wait: buildStructuredThreadReadModelWait(thread.wait),
    latestEpisode: buildStructuredThreadLatestEpisodeSummary(session, thread.id),
  };
}

export function buildStructuredThreadCurrentReadModel(
  session: StructuredSessionSnapshot,
  thread: StructuredThreadRecord,
...
export function buildStructuredThreadListReadModel(
  session: StructuredSessionSnapshot,
  input: BuildStructuredThreadListReadModelInput = {},
  ...
  const limit = clampStructuredThreadReadModelLimit(input.limit, 20, 100);
  return {
    threads: session.threads
      ...
      .toSorted(compareStructuredThreadsByAttention)
      .slice(0, limit)
      .map((thread) => buildStructuredThreadCompactRow(session, thread)),
  };
}
...
export function buildStructuredThreadGroupReadModel(
  session: StructuredSessionSnapshot,
  currentThread: StructuredThreadRecord,
): StructuredThreadGroupReadModel {
  return {
    threadGroupId: currentThread.threadGroupId as StructuredThreadGroupReadModel["threadGroupId"],
    currentThreadId: currentThread.id as StructuredThreadGroupReadModel["currentThreadId"],
    threads: session.threads
...
function buildStructuredThreadPendingReportRequests(
  session: StructuredSessionSnapshot,
  threadId: string,
  ...
  return (session.queuedMessages ?? [])
    ...
    .map((message) => ({
      queuedMessageId: message.id as StructuredThreadPendingReportRequest["queuedMessageId"],
      request: readQueuedReportRequestPayload(message.payloadJson) ?? "Report requested",
      createdAt: message.createdAt,
    }));
}

function readQueuedReportRequestPayload(payloadJson: string | null): string | null {
  if (!payloadJson) {
    return null;
  }
  try {
...
function buildStructuredThreadReadModelWait(
  wait: StructuredWaitState | null,
): StructuredThreadReadModelWait | null {
  if (!wait) {
    return null;
  }
  return {
...
function buildStructuredThreadLatestEpisodeSummary(
  session: StructuredSessionSnapshot,
  threadId: string,
): StructuredThreadReadModelEpisodeSummary | null {
  const episode = getLatestStructuredThreadEpisode(session, threadId);
  if (!episode) {
    return null;
  }
  return {
    id: episode.id as StructuredThreadReadModelEpisodeSummary["id"],
    title: episode.title,
    summary: episode.summary,
    createdAt: episode.createdAt,
  };
...
function getLatestStructuredThreadEpisode(
  session: StructuredSessionSnapshot,
  threadId: string,
): StructuredEpisodeRecord | null {
  return (
...
function compareStructuredThreadsByAttention(
  left: StructuredThreadRecord,
  right: StructuredThreadRecord,
): number {
  const statusDelta =
    structuredThreadStatusAttentionPriority(left.status) -
    structuredThreadStatusAttentionPriority(right.status);
  if (statusDelta !== 0) {
    return statusDelta;
  }
...
function structuredThreadStatusAttentionPriority(status: StructuredThreadStatus): number {
  switch (status) {
    case "waiting":
      return 0;
    case "troubleshooting":
...
function clampStructuredThreadReadModelLimit(
