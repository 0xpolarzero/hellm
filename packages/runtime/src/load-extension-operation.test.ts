...
import { Extensions, makeExtensions } from "@svvy/extensions";
...
describe("load_extension runtime operation", () => {
  it("invokes the package handler, applies actor binding update, refreshes generated context, and finishes the command", async () => {
    ...
    >;
    const extensions = await runTestEffect(makeExtensions());
    ...
    expect(updateCalls as unknown).toEqual([
      {
        target,
    ...
    expect(refreshCalls as unknown).toEqual([
      {
        scope: "target",
        target,
    ...
    expect(eventCalls).toEqual(["publish:1"]);
