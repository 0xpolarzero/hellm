...
describe("runtime queue steering", () => {
  it("marks the queued message at the front, publishes invalidations, and runs the post-commit lane", async () => {
    ...
    expect(harness.markCalls).toEqual([
  ...
  it("rejects queued messages that do not belong to the target before marking", async () => {
    const harness = createHarness({
      existing: queuedRecord({
    ...
    expect(harness.calls).toEqual(["lookup:queue_runtime_steer_01"]);
    expect(harness.markCalls).toEqual([]);
  });

  it("maps state, event, and post-commit failures to runtime contract errors", async () => {
    await expect(
      createHarness({ markFails: true }).run({
        target: handlerTarget,
        queuedMessageId: "queue_runtime_steer_01" as QueueItemId,
      }),
    ).rejects.toMatchObject({
    ...
    await expect(
      createHarness({ publishFails: true }).run({
        target: handlerTarget,
        queuedMessageId: "queue_runtime_steer_01" as QueueItemId,
      }),
    ).rejects.toMatchObject({
    ...
    await expect(
      createHarness({ postCommitFails: true }).run({
        target: handlerTarget,
        queuedMessageId: "queue_runtime_steer_01" as QueueItemId,
      }),
    ).rejects.toMatchObject({
