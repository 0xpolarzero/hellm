...
```bash
bunx smthrs revert workflow.tsx --run-id <id> --node-id implement --attempt 1
```
...
```bash
bunx smthrs timeline <run-id> --tree
bunx smthrs diff <run-id> <node-id>
bunx smthrs fork workflow.tsx --run-id <id> --frame 5 --reset-node analyze --label exp1
bunx smthrs replay workflow.tsx --run-id <id> --frame 5 --restore-vcs
```
