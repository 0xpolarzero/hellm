...
```bash
bunx smthrs init
bun add smthrs ai @ai-sdk/anthropic zod
...
bunx smthrs inspect <run-id>    # structured state
...
```tsx
/** @jsxImportSource smthrs */
import { createSmithers, Sequence, Task, AnthropicAgent } from "smthrs";
import { z } from "zod";
...
bunx smthrs inspect <run-id>                  # see the request
bunx smthrs approve <run-id> --node confirm-fix --by alice
...
- **Time travel** (replay a frame, fork a run, diff two runs): `bunx smthrs replay|fork|diff|timeline` — see [How It Works → Time travel](/how-it-works#time-travel).
