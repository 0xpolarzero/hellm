...
`bunx smthrs init` scaffolds a `.smithers/` folder preloaded with
production-ready workflows. Point your agent at one and go with
`bunx smthrs workflow run <id> --prompt "..."`:

```bash
# break a request into tickets, then implement them on a board
bunx smthrs workflow run tickets-create --prompt "add rate limiting and audit logging"
