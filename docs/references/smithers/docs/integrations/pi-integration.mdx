...
Drive Smithers server APIs from a PI extension or any Node process via `@smthrs/pi-plugin`:
...
Smithers does not provide a chat interface for PI. Chat UI integration is the responsibility of the host application using `@smthrs/pi-plugin`.
