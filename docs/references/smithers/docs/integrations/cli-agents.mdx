...
```tsx
import { ClaudeCodeAgent, Task, Workflow, createSmithers } from "smthrs";
import { z } from "zod";
...
Most built-in CLI agents support `bunx smthrs hijack <runId>`, which relaunches the agent in its native CLI session for interactive takeover.
