...
- [x] Store user-named Extension Managing snapshots and durable generated context bindings so historical surfaces remain inspectable after restart. Sources: `docs/progress.md`, `docs/specs/extension/extension_managing.extension.spec.md`.
  ...
  - [x] Mark existing orchestrator and handler surfaces stale by fingerprint after successful snapshot load, and drop removed user extensions from their stored loaded/available extension ids before the next opted-in pre-turn refresh.
...
- [x] Specify that generated `Agents.*` exports are persisted `TaskAgentParameters` records, `Agents.defineTaskAgent(parametersOrAgentsExport)` returns the Smithers-compatible `AgentLike` for `<Task agent={...}>`, and the returned AgentLike calls svvy through the narrow authenticated `runTaskAgent` workflow task-agent bridge. Sources: `docs/prd.md`, `docs/specs/workflow-library.spec.md`.
...
- [x] Add live tool projection tests for the implemented durable projection surface and split incremental streaming gaps into follow-up work. Sources: `docs/specs/live-tool-projection.spec.md`.
  ...
  - [ ] Follow-up: extend incremental streaming to specialized tools (`execute_typescript` source, native-control objective/report/question arguments, in-progress apply_patch patch-preview updates, approval-state live updates) with full projection and reload recovery coverage. Rationale: the current implementation omits delta snapshots for specialized tools since those tools manage their own command records during execution; adding streaming previews for those tools requires coordination between the streaming tracker and the specialized tool runtime.
