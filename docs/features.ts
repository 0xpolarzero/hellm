...
export const PRODUCT_FEATURES: ProductFeature[] = [
  ...
  },
  {
    ...
    status: "in-progress",
    summary:
      "Defines prompt composition around Agents and Extensions: agent profiles own model/reasoning, actor kind defaults, and sparse per-extension usage overrides selected from compact Agents-pane Loaded/Available/Off icon controls whose selected state uses a high-contrast app-standard active button shade while overridden rows underline the extension title in the accent color, remove stored entries when changed back to the resolved default, and link each row to its matching Extensions inventory record, while fixed app-native controls such as Extension Loading stay always default-loaded, and Extensions own builtin, user, and external_instruction capability records with ordered full instruction source files, per-file skip config that omits selected files from loaded prompt content without deleting or hiding them, one loaded instruction block assembled from non-skipped files, available minimal loading hints, generated previews, reset/delete controls appropriate to category, and requirement readiness surfaced through `list_extensions` and Extension Managing inspection. Extension snapshots include a default saved Initial baseline when no local snapshots exist, and user snapshots remain local restore points for source, package, usage, and coarse secret-state preservation. Base actor prompts are builtin instruction-only extensions: `base-common` is default-loaded for every adopted actor kind, while `base-orchestrator`, `base-handler`, and `base-workflow-task` are default-loaded by the corresponding default profile. The shared native thread-control implementation is exposed as two builtin agent-facing extension records: `thread-orchestration` default-loaded only for orchestrators and `thread-handling` default-loaded only for handler threads. Generated context previews show loaded base instruction extensions, non-skipped loaded capability extension instructions, available loading hints, read-only external instruction files such as `AGENTS.md` and `CLAUDE.md` with open-external-file controls, native tool declarations, loaded svvyx guidance, and emitted generated TypeScript client declarations; Incur-backed extensions are built behind one stable app-owned `svvyx <extension-id> ...` CLI, with usage state controlling generated guidance while user generated clients stay unavailable until sandboxed generated-client execution exists, and dependency-backed build readiness uses an app-global approval ledger keyed by exact dependency and trusted-dependency identities rather than shell approval, resumes blocked build and snapshot-load install work after approval, installs from a controlled app-owned package plan with lifecycle scripts disabled unless the exact trusted identity is approved, and validates exact installed package artifacts before runtime use; builtin prompt-only cx guidance is default-loaded for all adopted agent kinds and teaches official `cx` CLI use through `exec_command`, builtin prompt-only Git guidance is default-loaded for all agent kinds, builtin prompt-only GitHub guidance is default-loaded for orchestrators and handler threads and available for workflow task agents, these prompt-only CLI extensions use ordinary shell commands without wrapper tools or generated clients, and new surfaces bind to the latest ready generated agent context while existing surfaces receive durable `Update agent context` work when their fingerprint differs.",
    sourceSpecs: [
      "docs/prd.md",
      "docs/specs/extensions-and-tools.spec.md",
      "docs/specs/extension/svvyx-incur-runtime.spec.md",
  ...
  },
  {
    ...
    status: "in-progress",
    summary:
      "Provides an Agents pane between Logs and Extensions that owns app-wide orchestrator profiles, the delegated-handler `threadHandler` profile, and workflow-agent profiles, persists provider, model, reasoning, extension usage selections, per-profile extension instruction order, and profile metadata as app-global settings, uses builtin `base-*` instruction extensions for base role prompts, lets users create or duplicate additional orchestrator and workflow-agent profiles, deletes user-created profiles through an inline single-confirm action, drives the New orchestrator picker order and profile badges from the orchestrator-profile order, keeps the default orchestrator profile locked, first, non-draggable, and non-deletable while still allowing settings edits, keeps the default Explorer, Implementer, and Reviewer workflow agents non-deletable while still allowing edits and duplication, links each workflow-agent row to its exact `.agent.json` source file, lets profile-backed orchestrator sessions optionally save composer model and reasoning changes back to their profile for future sessions, uses internal title-naming settings for top-level session titles and handler-thread titles derived from delegated objectives without exposing title naming as a special profile, uses the delegated-handler `threadHandler` profile for delegated handler-thread surfaces with partial extension-state overrides available through `thread_start.threads[].extensions`, shows expanded orchestrator, handler, and workflow-agent profiles as one extension list with usage controls, open-extension links, expandable generated instruction text, Off rows at the end, active-row drag ordering, and reset-selection/reset-order actions, exposes focused-surface agent summaries in pane chrome, and uses direct-saving profile editors with connected-provider model dropdowns plus selected-model reasoning dropdowns derived from pi's normalized model metadata and runtime thinking controls rather than svvy-owned provider/model special cases.",
    sourceSpecs: [
      "docs/prd.md",
      "docs/specs/extension/thread_managing.extension.spec.md",
      "docs/specs/extensions-and-tools.spec.md",
