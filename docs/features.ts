...
export const PRODUCT_FEATURES: ProductFeature[] = [
  {
    ...
    status: "shipped",
    summary:
      "Runs svvy as a native desktop coding app where `@svvy/desktop` is the Electrobun/Svelte consumer of the runtime facade plus state read/command facades, `@svvy/pi-adapter` owns pi session and turn adaptation below the runtime, `@svvy/runtime` provides svvy-owned default workspace runtime scope behavior when no user workspace tabs restore, and non-authoritative desktop warm read-model caches feed static panes synchronously and update open panes immediately when app-global or workspace-keyed state changes.",
  ...
  },
  {
    ...
    status: "in-progress",
    summary:
      "Defines the seven public `@svvy/*` packages, generated `@svvyx/*` authoring outputs, Effect v4 service/layer composition, one app-owned `ManagedRuntime`, core-owned state-port boundaries backed by app-composed `@svvy/state` layers, runtime event boundaries, and package-boundary tests for the reusable main agentic flow below the desktop UI.",
    sourceSpecs: [
      "docs/prd.md",
  ...
  },
  {
    ...
    status: "in-progress",
    summary:
      "Runs scoped `@svvy/runtime` source invalidation coordinators for file-backed source inputs: one app-global coordinator watches Workflows and Extensions source roots, refreshes app-global generated `@svvyx/extensions` and `@svvyx/workflows` packages, and fans out workspace-link repair to acquired workspace runtime scopes; one workspace coordinator per acquired workspace watches external instruction candidates and discovered read-only host snippet Markdown sources for that workspace. Watcher events are only hints that trigger debounced and periodically reconciled deterministic source fingerprints; generated outputs and workspace package links are excluded as triggers; app-global agent settings, profile settings, and managed svvy snippets are DB/product-state-backed writes where `@svvy/state` commits facts and returns after-commit descriptors for runtime-owned typed notifications; refresh work rebuilds or rereads the smallest affected state through `@svvy/extensions` and `@svvy/state`, keeps the last ready generated output active when validation fails, surfaces source diagnostics through app logs/read models, publishes typed notifications so renderer read-model caches refetch affected state-backed read models, marks open surfaces stale solely by bound generated-context fingerprint mismatch, refreshes opted-in stale surfaces before the next prompt-bearing dispatch, and protects editable file-backed drafts with shared source-version compare-and-swap saves plus explicit keep-editing, discard-local, and overwrite-external conflict actions.",
  ...
  },
  {
    ...
    status: "shipped",
    summary:
      "Manages app-global model provider keys, OAuth-backed access with explicit healthy, expired, and refresh-failed credential health, icon-only provider key/OAuth/remove controls with tooltip explanations and inline remove confirmation, and a General settings surface for app appearance (`system`, `light`, or `dark`), the user's preferred external editor, the durable artifact directory, approval mode, default-on network access, and default-off ambient agent resource categories with `@svvy/state`-authoritative app-global persistence; expired or refresh-failed OAuth remains visible as connected credential state but is excluded from usable model availability and pi runtime auth until reconnect or successful refresh, while pi-backed agent profile configuration stays in the Agents pane, Web-specific TinyFish CLI auth stays in TinyFish CLI commands, and workspace-affecting operations stay on explicit `workspaceId`-routed requests.",
  ...
  },
  {
    ...
    status: "in-progress",
    summary:
      "Lets extensions declare required CLIs or command providers with a binary name, optional package name, default target version, version-check command, and reusable exact-version install/update command template when that provider is installed rather than `bunx`-resolved; reports missing, unknown, available, detected version, current version, latest version, and update-available state through Extension Managing inspect/build and the Extensions UI; makes build fail with ordinary structured errors when required CLI presence or version status is missing or unknown while using the detected installed version when one is available; lets the Extensions UI start app-global user-clicked install/update actions through `@svvy/runtime` using an immutable command plan validated by `@svvy/extensions`, a separate app-global extension dependency approval ledger, and sandbox writable roots limited to the app-owned extension package install/temp/cache areas, while optional origin workspace identity is UI lineage only; streams stdout/stderr from the tracked command into a closeable inline output panel under the clicked requirement while preserving pending, success, failure, command facts, app-log facts, and inventory-refresh feedback; agent-initiated installs remain ordinary Shell work; uses default versioned requirements for `cx-cli@0.7.1`, Smithers documentation generated from `smithers-orchestrator@0.22.0` while preserving official `bunx smithers-orchestrator ...` commands, and `@tiny-fish/cli@0.1.6`; and keeps Git and GitHub CLI requirements unversioned because their builtin instructions are not pinned to a specific CLI release.",
    sourceSpecs: [
      "docs/specs/extensions-and-tools.spec.md",
  ...
  },
  {
    ...
    status: "in-progress",
    summary:
      "Defines Smithers as a builtin prompt-only extension for handler-thread workflow authoring, generated from the current Extension Managing-selected Smithers documentation version with a small svvy boundary appendix; agents use official `bunx smithers-orchestrator ...` commands through Shell against workspace `.smithers/` packages, TypeScript source still imports from package `smithers-orchestrator`, reusable svvy assets are documented by the Workflows extension and imported only from generated `@svvyx/workflows`, and agents do not receive product workflow wrapper tools, Smithers runtime-control APIs or broad bridge tools, Smithers TypeScript facades, or workspace-local svvy workflow source guidance.",
    sourceSpecs: [
      "docs/prd.md",
  ...
  },
  {
    ...
    status: "in-progress",
    summary:
      "Defines one `@svvy/runtime` recovery coordinator per acquired workspace runtime scope, with duplicate same-cwd tabs sharing recovery state, app-wide auth/preferences kept outside workspace recovery, durable runtime recovery rows with transactional claims, leases, not-before/next-attempt timestamps, and idempotency keys for prompts, queues, initial handler starts, thread report notifications, report requests, request-input records, answer records, any nonblocking queued answer deliveries, waits, title jobs, generated `@svvyx/extensions` and `@svvyx/workflows` refresh plus separate workspace-link repair, and recovery observability, while renderer layout restore remains only a consumer of state-backed read models.",
