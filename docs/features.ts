...
export const PRODUCT_FEATURES: ProductFeature[] = [
  ...
  },
  {
    ...
    status: "in-progress",
    summary:
      "Runs one backend-owned source invalidation coordinator for file-backed product inputs, using watcher events only as hints that trigger debounced and periodically reconciled deterministic source fingerprints; watches app-global agent settings, Workflows source, Extensions source, external instruction candidates, and managed/discovered snippet roots while excluding generated outputs and workspace package links; rebuilds or rereads the smallest affected state, keeps the last ready generated output active when validation fails, surfaces source diagnostics through app logs/read models, refreshes renderer warm caches through backend-authored updates, marks open surfaces stale solely by bound generated-context fingerprint mismatch, and protects editable file-backed drafts with shared source-version compare-and-swap saves plus explicit keep-editing, discard-local, and overwrite-external conflict actions.",
  ...
  },
  {
    ...
    status: "in-progress",
    summary:
      "Lets extensions declare required global PATH CLIs with a binary name, optional package name, default target version, version-check command, and reusable exact-version install/update command template; reports missing, unknown, available, detected version, current version, latest version, and update-available state through Extension Managing inspect/build and the Extensions UI; makes build fail with ordinary structured errors when required CLI presence or version status is missing or unknown while using the detected installed version when one is available; lets the Extensions UI run user-clicked install/update actions directly from the app backend, streaming stdout/stderr into a closeable inline output panel under the clicked requirement while preserving pending, success, failure, and inventory-refresh feedback; agent-initiated installs remain ordinary Shell work; uses default versioned requirements for `cx-cli@0.7.1`, `smithers-orchestrator@0.22.0` exposing binary `smithers`, and `@tiny-fish/cli@0.1.6`; and keeps Git and GitHub CLI requirements unversioned because their builtin instructions are not pinned to a specific CLI release.",
    sourceSpecs: [
      "docs/specs/extensions-and-tools.spec.md",
  ...
  },
  {
    ...
    status: "in-progress",
    summary:
      "Represents reusable Smithers task-agent configuration as structured `TaskAgentParameters` records under `~/.config/svvy/workflows/agents`, generated as `Agents.*` exports such as `Agents.reviewerAgent`, `Agents.implementerAgent`, and `Agents.explorerAgent` in `@svvy/workflows`, with `Agents.defineTaskAgent(parametersOrAgentsExport)` returning the Smithers-compatible `AgentLike` for `<Task agent={...}>` and `Agents.TaskAgentParameters` also exported under the same namespace; the returned AgentLike calls svvy through one authenticated `runTaskAgent` bridge operation from a handler-thread command-scoped environment into the app-owned pi runtime, carrying task-agent params, Smithers taskContext/run/node/iteration/attempt identity, prompt/messages, rootDir, `workspaceSessionId`, and `sourceCommandId`, and returning `{ text, usage? }` plus optional app-supplied `output`; bridge calls are concurrent, bind one workflow-task-attempt surface each, expose no arbitrary app RPC/shell/settings/orchestrator controls, and do not duplicate Smithers workflow/run state; Agents-pane edits and `svvyx workflows save --kind agent` write the same structured source, and build validates provider/model/reasoning plus sparse extension usage overrides against pi-normalized provider metadata and generated `@svvy/extensions` instead of accepting freeform agent code.",
    sourceSpecs: [
      "docs/prd.md",
      "docs/specs/extensions-and-tools.spec.md",
      "docs/specs/workflow-library.spec.md",
  ...
  },
  {
    ...
    status: "shipped",
    summary:
      "Stores non-empty backend-accepted prompts per workspace after durable surface queue acceptance, and exposes shell-like recall in the composer.",
  ...
  },
  {
    ...
    status: "in-progress",
    summary:
      "Separates integrated app-chrome workspace tabs, shared durable workspace state, live surface runtimes, and Dockview-backed workspace layout slots, using one backend workspace runtime per canonical cwd with explicit `workspaceId` routing for every workspace-scoped request and sync event, never active workspace routing; keeps workspace tabs as chrome state that select `workspaceId` plus active layout id instead of owning durable layouts; opens a real svvy-owned default workspace tab when no user workspace tabs restore, with the same durable A/B/C layout behavior as user workspaces and the only default-workspace exception that an empty selected layout slot is seeded with one `Open Workspace` pane; lets `Open Workspace` retarget the current visual tab, `New Tab` create another default workspace tab over the shared default runtime and selected durable layout slot, and `Open Workspace in New Tab` create a selected user workspace tab; allows opening the same cwd in multiple visual workspace tabs that share the same runtime, session catalog, pi sessions, structured state, prompt queues, handler threads, app logs, workspace read models, saved Workflows generated-state visibility, and fixed durable layout slots keyed by `(workspaceId, layoutId)`; keeps workspace tabs left-aligned at the start of the main chrome, horizontally scrollable when crowded, draggable for user reordering, durably restored in user-defined order, and paired with compact icon controls plus colored running, unread, waiting, and error count badges shown only above zero with hover context; uses Dockview core for panels, groups, tabs, tab groups, splitters, drag/drop overlays, edge groups, floating groups, popouts, and serialized layout restore inside fixed workspace layout slots A, B, and C pinned at the far right while svvy stores panel-to-surface bindings and panel-local metadata in those slots; keeps empty user workspace layout slots muted but selectable, manages live pi surfaces in a shared registry keyed by `surfacePiSessionId`, gives each surface its own prompt lock, model or reasoning lifecycle, pending user message, queued follow-up messages, and surface-owned live assistant stream state, supports explicit open and close semantics, sidebar panel-location indicators, compact thread projections, and lets zero, one, or multiple panels attach to the same streaming surface without duplicating or cancelling the underlying runtime while keeping panel-local scroll independent per panel.",
    sourceSpecs: [
      "docs/prd.md",
