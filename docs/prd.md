...
Effect services, mutate extension source or state, derive affected surfaces, or replay post-hoc
runtime effects.

Runtime owns one app-global extension-source transaction coordinator shared by lifecycle and
TypeScript-API mutations, builtin/user source-edit saves, snapshot source capture/restore, and
extension lifecycle-journal recovery. Each complete filesystem mutation remains exclusive through
its source-fact/reconcile/finalize work, so two successful operations cannot silently erase one
another. Workflow source files use their separate workflow source authority.
...
`@svvy/state` owns the current extension registry, CLI and env readiness, and current-build
projections. The Extensions pane consumes those summaries through renderer-safe state read models;
file bodies are not inventory fields.
...
- state-backed registry and readiness projections derived from `@svvy/extensions` observations
- current-build evidence and build-attempt status
- typed source identities whose contents open lazily through versioned source-edit sessions, with
  generated schemas and declarations remaining read-only
