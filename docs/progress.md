...
- [x] Manage Extensions-pane default state and default order for future orchestrators and workflow task agents through shared `svvyx extensions defaults ...` state, with new user extensions loaded by default for those actors, handler defaults remaining owned by the Agents pane, duplicate/delete/reset controls, draggable default ordering, inventory filters, customized builtin tags, TypeScript API and instruction-token indicators, file-backed instruction editing with conflict handling, per-file skip controls, add/remove/reorder instruction-file lifecycle, app-owned trash for removed instruction files, and snapshots that preserve default order and default state. Commit(s): pending local changes
