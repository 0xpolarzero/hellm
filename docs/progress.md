...
- [x] Rebuild or reread only affected derived state after source fingerprints change, including Workflows package rebuilds for Workflows and extension source changes, renderer warm-cache refreshes for affected panes, and fingerprint-only stale prompt bindings for open surfaces whose generated context changes. Commit(s): `03bf43f69`
...
- [x] Mark existing orchestrator and handler surfaces stale by fingerprint after successful Extension Managing snapshot load, and drop removed user extensions from their stored loaded/available extension ids before the next opted-in pre-turn refresh. Commit(s): pending local changes
...
- [x] Add automatic generated agent context refresh for existing orchestrator and handler-thread surfaces through fingerprint-only stale detection, a checked-by-default update-before-next-turn intent, and backend pre-dispatch enforcement. Commit(s): pending local changes
  ...
  - [x] Record `Agent context updated` only for successful automatic pre-turn refreshes. Commit(s): pending local changes
