...
| `/settings/auth` | `docs/ui/ui.reference-screenshots/12-settings-auth.png` | `SettingsAuth.tsx` | Production-like settings visual source with fake providers | Settings shell with left nav, provider rows, OAuth connection rows, and app-managed credential status rows | Live inspection confirmed settings route renders provider rows and nav selection. Buttons hover but are visual-only. Theme button toggles `svvy-theme` localStorage and root dark class through shared hook | `providers`, `oauthConnections`, fake credential-status rows, hardcoded masked key, username, local Ollama URL | Map to provider auth settings, model discovery, stored provider state, OAuth state, extension env requirement status, and destructive confirmation where required. Do not port fake provider state or direct credential strings |
...
| `mockEpisodes` | One verified auth middleware episode with artifact links | Durable episode records, especially handler `thread_report` episodes |
...
| `providers`, `oauthConnections`, fake credential-status rows | Fake provider auth state, masked keys, connected username, credential presence | Provider auth settings, OAuth state, extension env requirement status, model discovery |
