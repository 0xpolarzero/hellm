...
- Persist session-agent choices separately from typed handler context packs: app-wide defaults for `defaultSession`, `quickSession`, and `namer`, per-session mode and prompt selection, and optional per-thread model, reasoning, and prompt overrides are structured settings facts, not transcript text.
