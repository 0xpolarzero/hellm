...
| `history`            | Records whether the handler started with product-filtered inherited orchestrator history (`forked`) or only handler prompt plus objective (`isolated`). |
...
- `forked` is the default for `thread_start.threads[]`. The handler is its own actor with handler
  system prompt, handler tools, handler extension binding, and a product-filtered inherited-history
  section from the current orchestrator surface before the delegated objective.
...
- `isolated` starts the handler with no inherited orchestrator conversation history. The handler
  receives handler system prompt, handler tools, handler extension binding, and the delegated
  objective.
