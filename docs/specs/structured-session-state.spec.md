...
| `history`            | Records whether the handler started with only handler prompt plus objective (`isolated`) or product-filtered inherited orchestrator history (`forked`). |
...
### Thread Start History Semantics

`history` records the creation-time context policy for the handler thread.

- `isolated` is the default for `thread_start.threads[]`. The handler receives handler system
  prompt, handler tools, handler extension binding, and the delegated objective, with no inherited
  orchestrator conversation history.
- command acceptance normalizes omitted `history` to persisted `"isolated"` on the handler thread,
  command payload, initial-start recovery work row, and `initial_handler_start` surface queue row.
  Omission is not a third durable history state.
- `forked` is an explicit opt-in. The handler is its own actor with handler system prompt, handler
  tools, handler extension binding, and a product-filtered inherited-history section from the current
  orchestrator surface before the delegated objective.
- `forked` history is delivered as a product-authored context block inside the handler's first
  prompt-bearing start item. It becomes part of the handler pi transcript for reproducibility, but it
  is not reconstructed as separate prior handler turns, not written into the handler system prompt,
  and not shared pi transcript state.
- `forked` history includes committed user messages and completed orchestrator assistant messages on
  the current transcript branch before the orchestrator assistant turn that calls `thread_start`,
  including the committed user message that caused that turn. It excludes tool calls, tool results,
  raw command output, logs, hidden reasoning, transient UI/control messages, queue rows, generated
  tool schemas, orchestrator-only callable surface details, the in-progress orchestrator assistant
  turn that called `thread_start`, any assistant prose in that turn, the `thread_start` tool-call
  content, and the `thread_start` result.
- `history` is creation-time provenance. It does not change when the handler later receives direct
  messages, report requests, thread follow-ups, or `thread_followup({ activate: true })`.
- thread read tools intentionally omit `history` from their compact thread refs. The boundary note in
  the handler's first start item tells the handler when inherited history was supplied, while the
  durable field remains product-state provenance for recovery, debugging, and projection.
