...
- Keep hard session deletion separate from archive: deletion is exposed only through the session row context menu, confirms in place by replacing `Delete` with `Confirm`, and removes the backing pi session history using pi's trash-first behavior.
