...
- `thread_start`, `thread_followup`, `thread_request_report`, `thread_report`, and
...
- Only `thread_report` with `outcome` concludes a handler objective and returns control to the orchestrator.
...
- `thread_followup({ activate: true })` lets the orchestrator explicitly re-engage a concluded handler objective for follow-up work in the same delegated context; Smithers run decisions remain inside that handler thread.
...
10. If the workflow reaches a state that needs another handler decision, `svvy` enqueues durable
    `workflow_attention` work for that same handler thread.
...
12. If the orchestrator later needs more help from a concluded handler objective, it uses `thread_followup({ activate: true })` to queue a new handler-surface message and reactivate the handler instead of directly controlling Smithers.
...
- derive no extension states from the owning handler thread, the handler profile,
  `thread_start` extension overrides, or handler report facts
...
- keep `thread_start`, `thread_followup`, `thread_list`, `thread_current`, `thread_group`,
  ...
  `smithers_*` out of workflow task-agent profiles, runtime tool schemas, and base prompts; workflow
  task agents cannot opt into the thread-control extensions in v1
...
| `smithers_run_workflow`               | required                            | Launch a discoverable workflow when `runId` is omitted, or resume exactly the supplied `runId`; omitted `runId` never silently resumes and is rejected when the same handler already owns a nonterminal run with the same `workflowId`. | Smithers semantic tool `run_workflow`, backed by embedded `runWorkflow(...)` plus stable v1 `launchRun` and `resumeRun` semantics. |
| `smithers_list_runs`                  | required                            | List recent runs and their compact summary state, while enriching each summary with svvy `sessionId` and `threadId` ownership when the run belongs to a recorded handler thread. | Smithers semantic tool `list_runs`, aligned with stable v1 `listRuns`, with svvy-side ownership projection layered on top. |
| `smithers_get_run`                    | required                            | Return the main run summary the handler needs to reason.                                                                                                            | Smithers semantic tool `get_run`, aligned with stable v1 `getRun`.                                                |
...
| `smithers_resolve_approval`           | required                            | Approve or deny a pending approval.                                                                                                                                 | Smithers semantic tool `resolve_approval`, aligned with stable v1 `submitApproval` semantics.                    |
...
| `smithers_get_run_events`             | required                            | Read raw lifecycle events and paginate by sequence.                                                                                                                 | Smithers semantic tool `get_run_events`, aligned with stable v1 `streamRunEvents` for live reads and durable event reads for recovery.                                              |
| `smithers_runs_cancel`                | required                            | Cancel a Smithers run using Smithers server semantics: direct terminal cancellation for `waiting-approval` and `waiting-timer`, request-and-abort for live `running`, and rejection for inactive states Smithers does not cancel directly. | Stable v1 `cancelRun`.                                                                                    |
| `smithers_signals_send`               | required when workflows use signals | Deliver a durable signal to a waiting run.                                                                                                                          | Stable v1 `submitSignal`.                                                                      |
| `smithers_frames_list`                | bridge or UI                        | Inspect rendered frames for timeline or inspector UIs.                                                                                                              | Smithers frame/DevTools storage and inspector bridge reads.                                                                    |
| `smithers_get_devtools_snapshot`      | bridge or UI                        | Read a DevTools tree snapshot for the workflow inspector.                                                                                                           | Smithers DevTools snapshot bridge read.                                                                                                              |
| `smithers_stream_devtools`            | bridge or UI                        | Stream DevTools tree deltas for a live inspector.                                                                                                                   | Stable v1 `streamDevTools`.                                                                                                                   |
...
- product read-model facts such as active workflow ownership, waiting projection, repair context, and compact sidebar states
...
`svvy` must not cache raw Smithers internals as an alternate execution database for product
decisions. It may persist normalized product projection fields such as `smithersStatus`, wait kind,
lineage, monitor cursor, and latest-event cursors on svvy-owned binding rows when those fields are
written from explicit Smithers bridge events or bootstrap/reconnect reads. Any operation that needs
current execution detail beyond that projection must re-read Smithers durable state by Smithers
identifiers.

### Thread Objective State During Supervision

Use thread objective state this way:

- `active` while the handler still owns the delegated objective
- `concluded` only after `thread_report` with `outcome` records a conclusion episode and no active workflow run remains owned by the current objective

A workflow run becoming terminal does not by itself conclude the handler objective.

Workflow failure or cancellation becomes handler-owned repair context. It does not return control to
the orchestrator unless the handler explicitly emits an update episode or concludes the objective.
...
- create a durable `workflow_attention` queue item for the owning handler `surfacePiSessionId`
- include the workflow run id, Smithers run id, workflow id, transition summary, reason, and
  idempotency key on that queued item
- let the normal surface queue runner claim the item in order and submit the derived synthetic user
  message to the same handler surface
- emit explicit workspace and surface updates so the renderer can follow the background work without polling

This is analogous to orchestrator reconciliation after `thread_report`, but it targets the handler thread instead of the orchestrator.
...
- ask the user
- emit an update with `thread_report`
- conclude with `thread_report` and `outcome`
...
After any direct paused-run terminalization, `svvy` must read the resulting Smithers events and run state, update the structured workflow-run projection cursor, clear product thread and session attention state as appropriate, and request handler attention for the cancellation.
...
2. decide which product projections are affected, such as handler attention, thread objective state, Project CI, inspector rows, logs, or related artifacts;
...
- app shutdown should cancel all monitors, but normal handler objective conclusion should not globally clear unrelated monitors

### Thread Conclusion Safety

`thread_report` with `outcome` should only conclude the current objective after the handler thread has resolved any active supervised run for that objective.
...
- a later `thread_followup({ activate: true })` on the same thread may start another workflow run for a new active objective under that thread
...
- replaying the same terminal workflow snapshot after a valid objective conclusion must not reactivate the objective
- replaying the same terminal workflow snapshot after a valid objective conclusion must not queue another handler-attention wake-up
- replayed terminal state may refresh run metadata, but it must not imply that a concluded objective still has active workflow ownership
