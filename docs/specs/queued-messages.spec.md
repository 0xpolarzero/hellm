...
commits, `@svvy/runtime` publishes committed invalidations. Insert paths that should attempt
immediate delivery issue only a primitive wake for the addressed surface queue after commit;
`queue-only` submit paths do not wake the surface queue. A primitive wake carries no queued-row
payload, dispatch result, callback target, or live turn state; queue dispatch re-reads authoritative
queue state before claiming. Consumers may observe queued, dispatching, pending, or active turn
state by refetching authoritative read models.
...
pi-native session/model objects. `@svvy/runtime` reads durable surface state, model settings, queue
...
Runtime queue acceptance is the commit point for ordinary composer sends. Acceptance durably creates
the queue item through `@svvy/state`, clears durable composer draft state, records prompt history
through the runtime-facing state port used by the acceptance transaction, and publishes typed
post-commit invalidations. Until acceptance returns, the renderer keeps only the live composer buffer
and writes no prompt history. Desktop clears or rebaselines the visible composer only after consuming
app/bootstrap-prepared renderer-safe notifications and refetching the authoritative composer, queue,
and prompt-history read models. Acceptance also invalidates older delayed renderer draft persistence
for that surface so stale pre-send text cannot be written back after runtime clears the draft.
Prompt-history refresh failures do not reject the accepted send.
...
by refetching authoritative queue/surface/turn read models after app/bootstrap-prepared
renderer-safe invalidations or stream patches derived from runtime events. Runtime events are
invalidation and live-patch signals; they are not durable queue, surface, or turn state.

Queued user messages can be removed or restored to composer before delivery only through
runtime/state-command-admitted actions persisted in `@svvy/state`; desktop invokes renderer-safe
facades and refetches read models. A durable row-level `Steer` action promotes a queued row into the
highest-priority next-delivery slot for that surface; it does not inject a prompt into the active pi
turn. Request-user-input answer rows claim ahead of ordinary user-message rows. Queue ordering and
priority facts survive restart and are honored by atomic `@svvy/runtime` queue claims.
...
`{ run, node, rootDir }`, and exactly one `promptSource` value as either `{ kind: "prompt" }` with a
non-empty prompt string or `{ kind: "messages" }` with non-empty user/assistant messages. Row-level
queue metadata carries `sourceCommandId`; the payload must not duplicate `sourceCommandId`, carry a
caller-supplied `threadId`, or carry a top-level `rootDir`. The
...
`@svvy/runtime` emits typed runtime events after commit. App/bootstrap derives renderer-safe
read-model invalidation notifications from those events. UI refresh status is derived from state
read models or app logs; it is not inserted as transcript prose. If the checkbox is disabled,
