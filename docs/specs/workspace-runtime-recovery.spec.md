...
- agent profiles, extension usage state, and generated agent context bindings
...
| Initial handler auto-start | handler-thread record plus initial-start recovery work row plus surface queue row | Claim exactly one initial-start recovery row per `threadId`, ensure the matching `initial_handler_start` surface queue row exists, then let the shared queue runner start the handler's first pi turn from the queued initial-start payload only if no accepted initial turn exists. Command acceptance normalizes omitted `history` to persisted `"isolated"` before these records are written; recovery state never treats omission as a third history mode. When normalized `history` is `"forked"`, that payload includes the product-filtered inherited orchestrator history plus the delegated objective; when normalized `history` is `"isolated"`, it includes only the delegated objective. Preserve the handler surface identity and generated agent context binding. |
