...
| `Fiber`, `FiberHandle`, `FiberMap`, `FiberSet`                          | Installed-export-audited; not production-adopted                                                            | Target primitives for runtime workspace/surface/turn workers, pi session scopes, and latest-job handles after the owning package adds exact manifest rows, package-boundary allowlists, owner policy, and focused tests                                                                                                                                                                                             | No detached fibers, no product ownership registry, no readiness inferred from fork or map presence; installed-export audit rows for `Fiber`, `FiberHandle`, `FiberMap`, and `FiberSet` are not production permission                                                                                                                                                                                                                                                                                                                      |
...
| `Crypto` | Strict service-member adoption | Production use is limited to exact service-instance policy rows. `@svvy/sandbox` owns `Crypto.Crypto.digest` for generated Seatbelt profile and packaged helper material. `@svvy/extensions` owns `Crypto.Crypto.digest` for source-version fingerprints and `Crypto.Crypto.randomUUIDv4` for source-edit atomic temp filenames. Runtime may provide/pass `Crypto.Crypto` through the layer graph without direct member calls, but that does not grant new runtime-owned crypto operations. Secure random bytes, UUIDv7 id generation, artifact/generated-package digests, tokens, and fingerprints require separate owner rows before production use. | Domain packages do not import host crypto directly. Abstract `Crypto` is not blanket permission for ids, tokens, signing, HMAC, fingerprints, or generated-package/artifact digests; each use must add exact members, owner globs, focused tests, and package-boundary expectations before use |
...
- `effect/Fiber` value imports are not production-adopted. Effect-lane test files may import
  audited conditional modules such as `effect/Fiber` only when the test directly verifies
  interruption, subscription cleanup, scoped lifecycle, or deterministic concurrency behavior.
  Production files remain blocked until exact member rows, owner policy, package-boundary
  allowlists, and focused tests exist.
...
| Source-gated interruption-region helper: `Effect.uninterruptible`; non-adopted helpers: `Effect.uninterruptibleMask`, `Effect.interruptible`, `Effect.onInterrupt`                                                                                                                                                                                                                                                                                                                                             | `effect/Effect`                                                                                                                                                                                                                                                       | `Effect.uninterruptible` is production-adopted only for the exact source-gated generated-package promotion section named in `packages/effect-adoption-manifest.ts`. Use short critical regions only where interruption would corrupt a durable lifecycle boundary, and keep the region limited to the commit/restore operations. Durable cancellation facts must still be written by the runtime command/turn lifecycle and recovery paths. Do not wrap pi turns, command execution, source scans, watcher loops, provider calls, user waits, or generated-package staging/build work in uninterruptible regions. `Effect.uninterruptibleMask`, `Effect.interruptible`, and `Effect.onInterrupt` remain non-adopted until exact manifest rows, owner policy, boundary allowlists, and focused tests promote them.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       |
...
- Effect service, worker, repository, stream, queue, handler, and generated-operation code does not
  ...
  `Cause` helpers. `Effect.exit` is not production-adopted unless exact manifest rows exist. Narrow
...
- Do not use JavaScript `try` / `finally` around `yield*` inside `Effect.gen` or `Effect.fn` for
  ...
  are visible. `Effect.uninterruptibleMask` and `SynchronizedRef.modifyEffect` are not production-adopted unless exact manifest rows exist and focused tests exist. JavaScript `try` / `finally` is allowed only in
...
- Do not import or model `RuntimeFlags`, `FiberRef`, or `FiberRefs` as package architecture
  ...
  production UTC timestamping uses adopted `DateTime.now` plus `DateTime.formatIso`, or explicit
  injected timestamp inputs. `DateTime.CurrentTimeZone`, `DateTime.withCurrentZone*`, and
...
- At bridge/test/process boundaries, use `managedRuntime.runPromiseExit(...)`, adopted
  ...
  and `Effect.exit` are not production-adopted unless exact manifest rows exist. v4 `Cause` is
...
- In Effect v4, `Ref`, `Deferred`, and `Fiber` values are not Effect subtypes. For
  ...
  `Deferred.await(deferred)`. `Fiber.join(fiber)` and `Fiber.await(fiber)` are audit-only until
  exact `effect/Fiber` production or test-only permissions are added. Do not `yield*` a `Ref`,
  `Deferred`, or `Fiber` value or pass those values to Effect combinators as if they were effects.
...
- `Fiber` from `effect/Fiber` is non-adopted for awaiting, joining, interrupting, or inspecting
  fibers already owned by the current scope, a `FiberHandle`, `FiberMap`, or `FiberSet`. It is not a
  separate ownership model for runtime work; untracked `Fiber` references must not be stored in
  state, service maps, renderer objects, or bridge payloads. Production imports require exact
  manifest rows first.
...
- `Semaphore.Semaphore.withPermit` is the only production-adopted permit helper, and only for
  ...
  bus publication lane and the runtime surface event publisher target-local stream cursor lane.
...
- Secrets loaded from process/config edges use process-local `Redacted.Redacted<T>` values only
  ...
  credential strings before passing them to `@svvy/pi-adapter`. `Config.redacted` remains
...
disallowEncode: true })` is the v4 API that forbids ordinary encode as well, but remains
  ...
  boundary that must hand the secret to a provider, subprocess, dependency installer, or pi model
  registry; production code must not value-import or call other `effect/Redacted` members.
