...
tool callback bridging, provider/model reads, and helper jobs are Effect services. Promise and
`AsyncIterable` APIs are edge facades only.
...
export class PiAdapter extends Context.Service<
  ...
  {
    sessions: {
      create(input: CreatePiSessionInput): Effect.Effect<PiSessionRef, PiAdapterError, Scope.Scope>;
      open(input: OpenPiSessionInput): Effect.Effect<PiSessionRef, PiAdapterError, Scope.Scope>;
      close(input: ClosePiSessionInput): Effect.Effect<void, PiAdapterError>;
    };
    turns: {
      run(input: RunPiTurnInput): Stream.Stream<PiRuntimeEvent, PiAdapterError>;
      interrupt(input: InterruptPiTurnInput): Effect.Effect<void, PiAdapterError>;
    };
    history: {
      restoreToEntry(
      ...
      forkFromEntry(
        input: ForkPiHistoryEntryInput,
      ): Effect.Effect<PiSessionRef, PiAdapterError, Scope.Scope>;
    };
    models: {
      list(input: ListModelsInput): Effect.Effect<ReadonlyArray<ModelInfo>, PiAdapterError>;
    };
    helperJobs: {
      generateTitle(input: GenerateTitleInput): Effect.Effect<GenerateTitleResult, PiAdapterError>;
    };
  }
>()("@svvy/pi-adapter/PiAdapter") {}
```

History methods declare their resource ownership explicitly. `restoreToEntry(...)` borrows the
already-scoped live handle for `input.session`, does not acquire a new pi handle, and returns the
same `PiSessionRef` plus updated opaque history refs. `forkFromEntry(...)` creates a new live pi
handle only for a runtime-allocated target `surfacePiSessionId`; its Effect therefore requires
`Scope.Scope`, persists the new opaque reference through `PiSessionReferencePort`, and releases the
handle with the caller-provided surface scope. Pi-adapter history methods must not create hidden
live handles outside runtime-owned surface scopes.

The primary layer consumes Effect services for provider auth, persisted pi session references, and
packaged pi runtime paths:
...
> = Layer.effect(PiAdapter, makePiAdapter);
```

`ChildProcessSpawner.ChildProcessSpawner` is required only for package-private pi protocol/helper
transports. If the installed pi runtime path does not need a helper process for a given operation,
the adapter does not spawn one; tests still provide a fake spawner layer so helper/protocol paths
remain deterministic when enabled.
...
pi ambient credentials directly. It may request live credential refresh only through
`ProviderAuthPort.refreshProviderCredentialSnapshot(...)`; expired or refresh-failed snapshots fail
session/turn/model operations with typed `PiAdapterError` and are not installed into pi auth storage.
...
`@svvy/pi-adapter`. Pi session creation, turn execution, history mutation, helper jobs, and runtime
prompt dispatch remain Effect service methods used by `@svvy/runtime` and package tests. If a
diagnostics-only facade is added, this spec must first name its exact method signatures, inputs,
outputs, package-probe use cases, and boundary tests; until then, pi-adapter probing stays on the
Effect service or package test fixtures.
...
reopen, and validates it through `PiSessionReferencePort.validatePiSessionReference(...)`. The
adapter must rebind the true pi `systemPrompt` channel from
`systemPromptBinding.text` before every prompt-bearing `turns.run(...)`, even when reopening an
existing session. It does not accept prior transcript prose, generated-context previews, or UI
...
history controls, model metadata reads, helper jobs, provider-auth port use,
pi-session-reference port use, and packaged runtime path port.
...
`Effect.Effect<NativeToolResult, RuntimeToolExecutionError>`. Conversion to pi `ToolDefinition[]`
happens inside adapter turn setup and is not exported as a public package API.
...
and is driven only from `RunPiTurnInput.toolExecutor`, which runtime backs with its package-private
accepted native tool runner. The adapter must not depend on desktop/headless facades, create or
receive a per-request `ManagedRuntime`, or expose a public callback-shaped package API for this
bridge. `PiAdapter.layer` does not require an app-bootstrap-provided callback runner because that
would require the app `ManagedRuntime` before the pi-adapter layer is acquired.

If pi requires a Promise-returning custom-tool callback, `@svvy/pi-adapter` constructs that callback
inside `turns.run(...)` by capturing the current turn Effect context and creating a turn-scoped
callback runner for only the Effect returned by `RunPiTurnInput.toolExecutor`. The runner primitive
is exactly Effect v4 `Effect.runCallbackWith(services)` or `Effect.runPromiseWith(services)`, where
`services` is the `Context` captured inside the active `turns.run(...)` scope. Bare
`Effect.runCallback`, bare `Effect.runPromise`, `ManagedRuntime.make`, `Layer.build*`, and
bootstrap-provided runner services are not allowed inside `@svvy/pi-adapter` service code. This is
the only allowed runner use in pi-adapter service code. It is treated as the pi callback bridge edge,
is released with the turn scope, never creates or receives a `ManagedRuntime`, never imports
desktop/headless facades, never runs arbitrary package effects outside the tool executor, and maps
callback aborts, typed failures, defects, and interruption through `Exit` into the pi callback
result. Package-boundary tests cover scope cleanup, abort mapping, typed `Exit` mapping, and the
absence of public callback or pi-native tool-definition exports. Public `@svvy/pi-adapter`
boundaries must not return pi-native tool definitions or callbacks.
...
The root `@svvy/pi-adapter` package entrypoint exports the `PiAdapter` Effect service and its public
layer factories. It exports no Promise facade, `AsyncIterable` facade, callback bridge, or edge
...
`@svvy/pi-adapter/messages` is the public adapter subpath for translating core-owned
`RuntimeSubmittedMessage` values into pi user messages. It may expose pi message-shaped structural
DTOs because its whole purpose is the pi boundary conversion. It must not type-alias or re-export pi
message classes/interfaces, session-manager handles, runtime facades, state ports, transport
handles, or app/bootstrap callbacks.

Native-tool conversion, managed-session bootstrap, pi handles, transport helpers, and pi-native
objects are package-private implementation details behind the `PiAdapter` service. No
`@svvy/pi-adapter/internal/*` subpath is a stable target-architecture import for app bootstrap,
...
desktop, generated-package, extension, sandbox, state, renderer/shared, and app-bootstrap production
code compose pi through the public `PiAdapter` service/layer and the `@svvy/pi-adapter/messages`
conversion subpath only.
...
`helperJobs.generateTitle(...)` uses an operation-scoped pi model call with its own
`systemPromptBinding`, model/reasoning, timeout, schema validation, and auth snapshot. It must not
call `prompt(...)` on the user surface session, append to user transcript, inherit active tools, or
reuse active-turn messages.
...
- Runtime owns a surface pi-session child scope keyed by `surfacePiSessionId`, implemented with
  `LayerMap.Service`, `FiberMap`, `FiberSet`, or `ScopedRef` according to the runtime spec. Adapter
  `sessions.create/open` acquire live pi handles into the caller-provided `Scope.Scope`; adapter APIs
  never expose pi-native handles.
...
- The adapter owns a package-private `PiLiveSessionRegistry` service keyed by the pi-free
  `PiSessionRef.sessionId` / `surfacePiSessionId` pair. Registry entries contain only scoped
  ...
  `turns.run(...)`, `turns.interrupt(...)`, `history.*`, and `helperJobs.*` may borrow a live entry
  only after validating the pi-free `PiSessionRef` against `PiSessionReferencePort`; they must fail
  with a typed `PiAdapterError` when the durable reference exists but no live scoped handle is
  currently acquired. Runtime owns whether to reopen the session or fail the turn; the adapter does
...
- `turns.run(...)` borrows the scoped live handle for the stream lifetime, installs turn
  subscriptions with acquire/release finalizers, and returns a
  `Stream.Stream<PiRuntimeEvent, PiAdapterError>`.
- Pi callback/event APIs are adapted with `Stream.callback` plus scoped acquire/release finalizers
  or an equivalent adopted v4 scoped stream constructor. The bridge owns a bounded queue,
  unsubscribe/abort finalizers, and typed overflow failure mapping; it must not expose unbounded
  callback buffers, orphan subscriptions, or background promises outside the stream scope.
...
| `tool_execution_update`          | none                          | `pi.tool_execution.updated`                                          | runtime may append command progress/result snapshots                                        |
...
- `turns.interrupt({ surfacePiSessionId, turnId })` calls the checked-in pi session interruption API
  (`AgentSession.abort()` when using `AgentSession`) and waits for pi to become idle before the turn
  stream finalizes. Runtime may also interrupt the `turns.run(...)` fiber; the adapter finalizer must
  invoke the same pi abort/idle path. Cancellation maps to normalized `PiRuntimeEvent`/`PiAdapterError`
  output sufficient for runtime to terminalize the turn as cancelled and ignore late tool/output
  events. Interrupting a turn does not close the owning session handle unless the session or surface
...
- A package-private `PiSessionFactory` service owns pi-native construction details:
  `SettingsManager`, `SessionManager`, `ModelRegistry`, `ResourceLoader`, and any installed-version
  adapter shims. Public `PiAdapter` methods call that factory; runtime, app/bootstrap, and tests do
  not receive pi-native constructors or mutate pi settings directly. The factory is provided by
  `PiAdapter.layer` and is scoped to the layer unless a later spec names a concrete cache/resource
  owner.
...
- Helper jobs that produce product facts but must not mutate the user transcript, such as title
  ...
  their own explicit `systemPromptBinding`, model/reasoning selection, timeout, and typed output
  schema.
...
| Operation-scoped helper pi session       | `@svvy/pi-adapter` helper job service                               | host resource | `operationScoped`                                                        | title/helper operation with explicit prompt binding, model/reasoning, timeout, and schema       | helper operation finalizer                                                                  | no                                                                           | interruption aborts helper job and closes helper session; user transcript is not mutated                                                      | title helper isolation test, timeout/interruption cleanup test                    |
| Protocol/helper child process, when used | `@svvy/pi-adapter` package-private transport                        | host resource | `keyedLayerMapScoped` or `operationScoped` matching the owning pi handle | adapter transport acquisition inside the owning pi session/helper scope                         | owning scope close interrupts process, reader/writer/heartbeat fibers, and pending requests | yes only within the owning handle scope                                      | process exit or reader failure fails pending deferreds with typed errors and releases fibers                                                  | stdio/protocol fixture test, pending request cleanup test                         |
...
- keep pending protocol requests in a scoped `Ref` or `SynchronizedRef` map from request id to
  one-shot `Deferred`
...
`turns.run(...)` submits this as one real pi prompt with prompt-template expansion disabled.
...
const events = pi.turns.run({
  ...
  model: { providerId: "openai" as ProviderId, modelId: "gpt-5.4" as ModelId },
...
`PiHistory` exposes pi transcript mechanics through svvy refs:
...
ownership. `restoreToEntry(...)` borrows the existing scoped live handle for `input.session`,
validates the persisted reference, moves that same pi session to `entryId`, and saves an updated
same-surface `PiSessionReference` only if pi returns a changed opaque reference. It does not create
a new surface, new scope, queue row, or prompt.

History operations are serialized against the live `PiLiveSessionRegistry` entry. If a turn stream
is active for the addressed session, `restoreToEntry(...)` and `forkFromEntry(...)` fail with typed
`PiAdapterError` instead of mutating pi history under an active prompt. Adapter history methods
mutate only pi-owned history/reference state; runtime owns any queue, turn, transcript, or product
surface changes that make a history operation visible to users.

`forkFromEntry(...)` requires runtime to have already created the durable target surface identified
by `targetSurfacePiSessionId`. The adapter borrows the source scoped live handle, asks pi to fork
the history at `entryId`, acquires the forked target live handle in the caller-provided target
surface scope, and persists the new target `PiSessionReference` keyed by
`targetSurfacePiSessionId`. Source and forked sessions share no live handle after the fork. The
adapter must not invent target surface ids, attach a fork to the source scope, or store fork
ownership outside `PiSessionReferencePort`.
...
`PiModelRegistryProvider` and joins provider status from
`ProviderAuthStatusStatePort.listProviderStatuses({ workspaceId })`. It returns only core `ModelInfo` rows. It
does not return pi model objects, base URLs, provider headers, raw credential material, price/cost
metadata, registry override internals, or display-only summaries that can be derived by the caller.
When `providerId` is omitted, it returns all known pi providers. When the auth port returns no row
for a listed provider, the adapter returns `authStatus.health: "missing"` for that provider.
...
The target public `@svvy/pi-adapter` entrypoint exports only the `PiAdapter` service and layer
factories. Pi-free input/output contracts and typed boundary errors are imported from `@svvy/core`;
the adapter does not redefine or re-export duplicate contract names. Root `makePiAdapter` is not a
public service constructor. Any module that imports pi-native symbols is package-private to
`@svvy/pi-adapter`. App bootstrap, runtime, desktop, generated declarations, generated packages, and
shared app contracts compose the adapter only through the public `PiAdapter` service/layer and
core-owned port contracts. Boundary tests fail if public declarations mention pi-native session,
event, model, tool, resource-loader, transport, protocol, or child-process handle types.
...
- adapter services for pi session creation/open/close and history operations
...
App-entry session catalog modules are app/bootstrap host-adapter code, not target public ownership
surfaces for `@svvy/pi-adapter` APIs.
...
```ts
export const layerTestFakePi: Layer.Layer<PiAdapter | PiAdapterTestHarness, never, never>;
```

`PiAdapterTestHarness` exposes semantic receipts for session open/create/close, system-prompt
delivery, pi-native resource disabling, turn stream events, history restore/fork calls, model
metadata reads, helper jobs, and scope finalizers. Tests use the harness receipts and fake pi
services; they do not create manual `ManagedRuntime` instances or expose pi-native objects through
