...
export class Runtime extends Context.Service<
  Runtime,
  {
    ...
    generatedContext: RuntimeGeneratedContextService; // transient read-only profile/workflow-agent preview query
    extensions: RuntimeExtensionsService; // public build orchestration group; package-local service shape over core-owned DTOs
    sourceEdits: RuntimeSourceEditsService; // includes accepted atomic extension TypeScript-API configuration; public service/facade group; group type stays package-local
...
const runtimeRequestInputWaitLayer = layerRuntimeRequestInputWaitService;
...
rows. The runtime service's non-facade workspace-recovery lane uses `"startup-recovery"` only after
a durable `queue_delivery` recovery row is claimed. Adding a new wake reason requires the same
change to update the reason union,
...
- `RuntimeQueueWakeService` is the runtime-local wakeup scheduler for already committed surface
  ...
  commits an allowed internal queue row for the addressed surface. `startup-recovery` is emitted
  only by the runtime service's non-facade workspace-recovery lane after a claimed durable
  `queue_delivery` row resolves its addressed surface. No other caller may reuse these reasons as a
  generic "queue changed" signal. The wake result returns no queued-row payload,
  dispatch result, or derived read-model data; UI consumers refetch DB-backed read models from
  runtime notifications. Before dispatch, the service resolves the target's owning `WorkspaceId`
  through `RuntimeWorkspaceStatePort.resolvePromptTargetWorkspaceId(...)`; it never casts or
  otherwise treats `PromptTarget.workspaceSessionId` as a `WorkspaceId`. Workspace-scoped
...
- `RuntimeRequestInputWaitService` owns blocking request-input waiting plus post-answer and
  ...
  `RuntimeCommandStatePort`, `RuntimeSessionWaitStatePort`, and the process-local blocking request
  wait registry. The public answer lifecycle supplies the composed `RuntimeQueueWakeService`
  callback only after the answer commit and invalidation publication have succeeded, avoiding a
  service-layer cycle between accepted tool execution, queue dispatch, and queue wake. Its
  package-private shape is:
  ...
  type RuntimeRequestInputWaitService = {
    ...
    afterAnswerCommitted(input: {
      ...
      target: PromptTarget;
      wakeSurface: RuntimeQueueWakeServiceService["wakeSurface"];
    }): Effect.Effect<void, RuntimeContractError>;
  ...
  behind this service and its registry/state implementation. Blocking request creation atomically
  commits the request/questions, linked command waiting facts, and durable user wait. Terminal
  answer/default/cancel uses one durable compare-and-set transaction for request, answer, linked
  command, and matching wait facts. Runtime publishes that one returned `afterCommit` batch through
  `RuntimeEventBus` before touching the live waiter. The public app/runtime facade does not expose
  separate timeout, recovery, cancellation, command-finalization, wait-finalization, or
  ...
  The service does not publish renderer payloads or return request-input read-model payloads.
  Answer wake behavior comes from the committed answer result rather than a post-answer
  `RuntimeRequestStatePort.getRequestInput(...)` read; lifecycle-owned timeout, cancellation, wait,
  and command writes publish only their own committed invalidation descriptors.
  A duplicate client-submission answer returns the original durable delivery receipt with no new
  invalidations. Nonblocking duplicates do not enter `afterAnswerCommitted(...)` again and cannot
  wake the same queue row twice. Blocking duplicates may re-enter only the idempotent live-waiter
  delivery path so a client retry can resolve a matching process-local handle from already-terminal
  durable facts; they perform no state follow-up writes.
...
- `RuntimeApprovalWaitService` owns process-local approval waiters inside the runtime-owned accepted
  ...
  request, command waiting facts, and matching session wait in one `RuntimeApprovalStatePort`
  transaction. The service is a runtime-owned waiter registry, not a state writer: it resumes or
  cancels the active tool call only after the durable approval transition has committed and
  published. The service instance is constructed inside
...
- `RuntimePromptExecutionService` materializes a claimed queued message into pi work through
  `@svvy/pi-adapter`, executes inside the surface scope's retained active-turn fiber, and owns stream consumption, tool-call acceptance,
...
The dependency order is strict. `RuntimeQueueWakeService` owns queued-answer wake scheduling;
`RuntimeRequestInputWaitService` owns blocking wait resolution and timer rescheduling, with the
answer lifecycle passing the wake callback after commit/publication. Accepted native-tool
...
Startup readiness restores blocking waits for stores registered when the app runtime is acquired.
Every successful `workspaces.acquire(...)` and `workspaces.acquireDefault(...)` reruns the same
idempotent restore after the workspace state store is registered and its live workspace scope is
acquired, so workspaces opened after app startup cannot strand durable blocking requests.
...
`workspaces`, `surfaces`, `messages`, `queues`, `requestInput`, `generatedContext`, `commands`, `approvals`,
...
contracts come from `@svvy/core`; RuntimeEffectRequest appliers, app-private response-bearing
`svvyx` request adapters, event-bus internals, prompt-execution handles, prompt-execution-context construction,
...
`RuntimeGeneratedPackageRefreshHostPortService`, `RuntimeExternalInstructionScanInputPort`,
`RuntimeExternalInstructionScanInputPortService`, `RuntimeSourceInvalidationScanPort`,
...
`RuntimePrimitiveToolHostPort`, `RuntimePrimitiveToolHostPortService`,
...
runtime admission, waits for tracked queue-claim and accepted native-tool calls that crossed the
admission boundary, drains or terminalizes admitted queue, command, request-input, and task-agent
...
Admission bookkeeping does not serialize independent admitted operations. A short semaphore lane
increments the in-flight count before each public runtime operation, queue claim, or non-waiting
accepted native-tool operation starts and
atomically closes the gate when shutdown begins. The shutdown leader waits for that exact admitted
set to release before it starts the shutdown drain effect or completes the shared receipt. Late
accepted native-tool calls fail before launch-policy acquisition, approval admission, request
creation, extension loading, or handler-thread creation can run.
Accepted approval and blocking request-input methods gate their admission but do not retain the
in-flight count across the runtime-owned wait; shutdown preparation owns cancelling those waits
before forced prompt recovery.
...
durable state.
When the bounded graceful prompt drain expires, forced active-prompt interruption is a joined
barrier: runtime waits for the retained prompt fiber and its finalizers to finish before it writes
durable interrupted-turn recovery facts, cancels recovered command handles, publishes recovery
invalidations, or returns the shutdown receipt.
...
| `messages.submit`                             | `SubmitMessageInputSchema`                                                                                               | `SubmitMessageResultSchema`                                                                                                                                                   | `invalid-input`, `target-not-found`, `target-not-ready`, `stale-state`, `state-conflict`, `backpressure`, `runtime-shutdown`, `runtime-disposed`           | `RuntimeQueueStatePort.acceptSubmittedSurfaceMessage(...)` atomically records the queue row, durable draft cleanup, and exact non-empty workspace prompt-history text.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             | descriptor-derived `workspace_read_model.changed` / `app_read_model.changed`; no `queue.changed` event is emitted by this public method unless a queue lifecycle publisher commits and publishes that event from authoritative queue state | `surface`, `sessionNavigation`, `promptHistory`                                                                                              | enqueue-and-run, queue-only, client-submission idempotency including terminal replay, exact prompt text, renderer-field rejection, after-commit publication                                                 |
| `messages.abort`                              | `AbortPromptInputSchema`                                                                                                 | null                                                                                                                                                                          | `invalid-input`, `target-not-found`, `stale-state`, `state-conflict`, `runtime-shutdown`, `runtime-disposed`                                               | `RuntimeQueueStatePort.cancelSurfaceMessage(...)` with a queued/steering status guard for queued rows; claimed/dispatching rows are rejected so callers use active-turn cancellation for already-dispatching prompts; active-turn cancellation interrupts the retained runtime-owned prompt fiber and commits queue/turn/dangling-command terminal facts through atomic `RuntimeTurnStatePort.settlePromptTurn(...)` or `recoverInterruptedTurn(...)`                                                                                                                                                                                                                                                                                                              | `queue.changed`, `surface.stream`                                                                                                                                                                                                          | `surface`, `commandInspector`, `requestInput`                                                                                                | queued abort, claimed queued-row rejection, active-turn abort, all-for-surface abort, terminal idempotency                                                                                                  |
...
| `approvals.answer`                            | `AnswerRuntimeApprovalInputSchema`                                                                                       | `AnswerRuntimeApprovalResultSchema`                                                                                                                                           | `invalid-input`, `target-not-found`, `stale-state`, `state-conflict`, `runtime-shutdown`, `runtime-disposed`                                               | `RuntimeApprovalStatePort.getApprovalRequest(...)`, then one `RuntimeApprovalStatePort.resolveApprovalRequest(...)` transaction that resolves the approval, transitions the linked command, and clears the matching approval wait; runtime publishes that transaction's complete descriptor batch before calling package-private `RuntimeApprovalWaitService.afterApprovalCommitted(...)` inside the acquired `Runtime.layer`                                                                                                                                                                                                                                                                                                                                      | `workspace_read_model.changed`, `command.changed`                                                                                                                                                                                          | `runtimeApprovals`, `commandInspector`, `sessionNavigation`; `surface` when the approval state-port descriptor includes the affected surface | approve/deny, stale answer, atomic command/wait finalization, idempotent replay                                                                                                                             |
| `requestInput.setVariant`                     | `SetRequestInputVariantInputSchema`                                                                                      | `SetRequestInputVariantResultSchema`                                                                                                                                          | `invalid-input`, `state-conflict`, `runtime-shutdown`, `runtime-disposed`                                                                                  | `RuntimeRequestStatePort.setRequestInputVariant(...)`, publish committed descriptors, then refresh generated context for every currently acquired workspace with reason `profile-settings-changed` so variant-specific declarations and instructions are regenerated                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               | descriptor-derived `app_read_model.changed`, followed by any descriptor-derived generated-context changes                                                                                                                                  | `requestInput`; later `surface` / `extensions` only when a generated-context binding changes                                                 | strict facade decoding, commit-before-publication, deterministic acquired-workspace refresh order, variant-aware prompt declarations                                                                        |
| `requestInput.setBlockingTimeout`             | `SetRequestInputBlockingTimeoutInputSchema`                                                                              | `SetRequestInputBlockingTimeoutResultSchema`                                                                                                                                  | `invalid-input`, `state-conflict`, `runtime-shutdown`, `runtime-disposed`                                                                                  | `RuntimeRequestStatePort.setRequestInputBlockingTimeout(...)`, then publish committed descriptors; timeout changes do not refresh generated context because timeout policy is read from committed state when an invocation begins                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  | descriptor-derived `app_read_model.changed`                                                                                                                                                                                                | `requestInput`                                                                                                                               | strict facade decoding, commit-before-publication, no generated-context refresh                                                                                                                             |
| `requestInput.answer`                         | `AnswerRequestInputInputSchema`                                                                                          | `AnswerRequestInputResultSchema`                                                                                                                                              | `invalid-input`, `target-not-found`, `stale-state`, `state-conflict`, `backpressure`, `runtime-shutdown`, `runtime-disposed`                               | one `RuntimeRequestStatePort.answerRequestInput(...)` transaction records the answer and, for the final blocking answer, terminalizes the linked command and clears the matching user wait; nonblocking answers may commit a `request_user_input_answer` queue row; after descriptor publication `RuntimeRequestInputWaitService.afterAnswerCommitted(...)` only resolves the process-local `Deferred` or wakes the nonblocking queue                                                                                                                                                                                                                                                                                                                              | always `workspace_read_model.changed`; `queue.changed` only when `delivery.kind === "nonblocking-queued"` and a `request_user_input_answer` queue row was committed                                                                        | `requestInput`, `surface`, `commandInspector`; queue read-model invalidation only for committed queue delivery                               | blocking open, atomic blocking resolution, nonblocking queued, nonblocking recorded-only, duplicate submission                                                                                              |
...
| `sourceEdits.save`                            | `RuntimeSaveExtensionSourceEditInputSchema`                                                                              | `SourceEditSaveResultSchema`                                                                                                                                                  | `invalid-input`, `dependency-not-ready`, `target-not-found`, `read-only-source`, `stale-state`, `state-conflict`, `runtime-shutdown`, `runtime-disposed`   | `Extensions.sources.openEditSession(...)`, `RuntimeSourceStatePort.readSourceVersion(...)`; workflow-agent saves use app-global `RuntimeLayerModelResolverPort` plus `RuntimeLayerProviderAuthPort` before `Extensions.sources.saveEditSession(...)`, then atomically call `RuntimeSourceStatePort.recordWorkflowAgentSourceSave({ source, observation })`; other source kinds retain `recordSourceSave(...)`; runtime publishes committed descriptors, hints `RuntimeSourceInvalidationService`, and enqueues source-owned `source_reconcile` recovery on post-file failure                                                                                                                                                                                       | descriptor-derived `app_read_model.changed` / `workspace_read_model.changed`                                                                                                                                                               | `extensions`, `agents`, `workflowsGenerated`                                                                                                 | runtime wrapper decoding, compare-and-swap/stale/overwrite, exact provider/model/reasoning/auth rejection before write, atomic valid workflow-agent observation, file-write/state-fail recovery             |
| `sourceEdits.createWorkflowAgent`             | `RuntimeCreateWorkflowAgentSourceInputSchema`                                                                            | `WorkflowAgentSourceLifecycleResultSchema`                                                                                                                                    | `invalid-input`, `dependency-not-ready`, `state-conflict`, `runtime-shutdown`, `runtime-disposed`                                                          | app-global `RuntimeLayerModelResolverPort` plus `RuntimeLayerProviderAuthPort`, `Extensions.sources.createWorkflowAgent(...)`, `RuntimeSourceStatePort.readSourceVersion(...)`, atomic `RuntimeSourceStatePort.recordWorkflowAgentSourceSave({ source, observation })`, committed descriptor publication, workflows invalidation hint, and source-owned recovery enqueue on post-file failure                                                                                                                                                                                                                                                                                                                                                                      | descriptor-derived `app_read_model.changed`                                                                                                                                                                                                | `agents`, `workflowsGenerated`                                                                                                               | facade forwarding, reserved/default id and blank-label rejection, exact model/auth/reasoning, unknown/ineligible extension references, matching valid observation, no-clobber publication, recovery payload |
| `extensions.build`                            | `BuildRuntimeExtensionInputSchema`                                                                                       | `BuildRuntimeExtensionResultSchema`                                                                                                                                           | `invalid-input`, `target-not-found`, `target-not-ready`, `dependency-not-ready`, `state-conflict`, `stream-failed`, `runtime-shutdown`, `runtime-disposed` | Input requires a client request id. Runtime serializes by extension id and reads its durable request attempt first. Identical successful replay returns the matching current manifest without a process or new attempt; identical failed replay rethrows the stored coarse classification; running replay resumes the same attempt; conflicting request reuse fails. New work obtains exact registry/source observations, commits them, starts one durable request-bound attempt, calls `Extensions.builds.build(...)`, and records terminal success or classified failure. Every failure or interruption after attempt start terminalizes that attempt exactly once. No raw process output enters state.                                                          | only exact `afterCommit` descriptors returned by registry/evidence/attempt/success/failure state commits; publication after a terminal commit cannot rewrite the terminal receipt                                                          | app `extensions`, narrowed to the affected extension for attempt/terminal mutations                                                          | strict facade DTOs, success order, durable succeeded/failed/running/conflict replay across reopen, exact failure classification, start-publication failure terminalization, same-extension serialization    |
| `sourceEdits.duplicateWorkflowAgent`          | `RuntimeDuplicateWorkflowAgentSourceInputSchema`                                                                         | `WorkflowAgentSourceLifecycleResultSchema`                                                                                                                                    | `invalid-input`, `dependency-not-ready`, `target-not-found`, `state-conflict`, `runtime-shutdown`, `runtime-disposed`                                      | `Extensions.sources.openEditSession(...)`, canonical source decode, app-global `RuntimeLayerModelResolverPort` plus `RuntimeLayerProviderAuthPort`, `Extensions.sources.duplicateWorkflowAgent(...)`, `RuntimeSourceStatePort.readSourceVersion(...)`, atomic `RuntimeSourceStatePort.recordWorkflowAgentSourceSave({ source, observation })`, committed descriptor publication, workflows invalidation hint, and source-owned recovery enqueue                                                                                                                                                                                                                                                                                                                    | descriptor-derived `app_read_model.changed`                                                                                                                                                                                                | `agents`, `workflowsGenerated`                                                                                                               | copied model admission, current extension-reference revalidation, exact overrides/order preservation, matching valid observation, no clobber, scan/reconcile recovery                                       |
| `sourceEdits.deleteWorkflowAgent`             | `RuntimeDeleteWorkflowAgentSourceInputSchema`                                                                            | `WorkflowAgentSourceDeleteResultSchema`                                                                                                                                       | `invalid-input`, `target-not-found`, `stale-state`, `state-conflict`, `runtime-shutdown`, `runtime-disposed`                                               | `Extensions.sources.openEditSession(...)`, `RuntimeSourceStatePort.readSourceVersion(...)`, `Extensions.sources.deleteWorkflowAgent(...)`, atomic `RuntimeSourceStatePort.recordWorkflowAgentSourceDelete({ source })` with path/previous version/fingerprint, committed descriptor publication, workflows invalidation hint, and source-owned scan/reconcile recovery                                                                                                                                                                                                                                                                                                                                                                                             | descriptor-derived `app_read_model.changed`                                                                                                                                                                                                | `agents`, `workflowsGenerated`                                                                                                               | default-agent rejection, stale delete, atomic tombstone, latest-disk recovery, divergent retry rejection                                                                                                    |
...
`"message-submitted"`, `"request-input-answer-queued"`, `"queue-steered"`,
`"runtime-queue-inserted"`, or `"startup-recovery"`. It must not receive queue row payloads,
dispatch results, pi handles,
...
`setTurnDecision`, `settlePromptTurn`, and `recoverInterruptedTurn`. Ordinary standalone queue
terminal methods remain available for non-turn queue work, but prompt completion uses the one
claim-fenced `settlePromptTurn` transaction. The core symbol index contains schemas for the claim
input, claimed queue item result, claim lease, `StartRuntimeTurnInput`, `RuntimeTurnRecord`, and
prompt settlement input/result.
...
Runtime consumes `PiRuntimeEvent` values sequentially in
`runtime-prompt-execution-service.ts#handlePiRuntimeEvent`. Every renderer patch first advances the
durable `RuntimeTranscriptStatePort` cursor by compare-and-set; the returned cursor sequence is then
passed unchanged to `RuntimeSurfaceEventPublisher`. Product `MessageId` values always come from the
transcript state port. Pi message refs, provider response ids, and Pi history entry ids never become
product message ids.

| `PiRuntimeEvent.type`            | Durable state action                                                                                                                                                                              | Live/read-model projection                                                                       |
| -------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------ |
| `pi.user_message.committed`      | `commitUserMessage(...)`, then `bindPiHistoryEntry(...)` when Pi persistence produced a history ref                                                                                               | one `surface` invalidation and `user_message_committed` at the committed cursor                  |
| `pi.assistant_message.started`   | `beginAssistantMessage(...)`; records API/provider/model/start time and maps the Pi ref to the returned `MessageId`                                                                               | one `surface` invalidation and `assistant_message_started`                                       |
| `pi.assistant.text.delta`        | `appendAssistantContentDelta(..., kind: "text")`                                                                                                                                                  | no read-model invalidation; `assistant_text_delta` uses the durable cursor sequence              |
| `pi.assistant.thinking.delta`    | `appendAssistantContentDelta(..., kind: "thinking")`                                                                                                                                              | no read-model invalidation; `assistant_thinking_delta` uses the durable cursor sequence          |
| `pi.tool_call.started`           | create/reuse the command, `upsertAssistantToolCall(...)`, then `linkAssistantToolCallCommand(...)`                                                                                                | command descriptors plus ordered `tool_arguments_snapshot` and `active_command` patches          |
| `pi.tool_call.arguments.delta`   | record the command argument snapshot and `upsertAssistantToolCall(...)` with accumulated JSON                                                                                                     | command descriptors and `tool_arguments_snapshot`; transcript write itself emits no invalidation |
| `pi.tool_call.accepted`          | create/reuse the command, upsert immutable accepted arguments, and link the command if the start event did not already do so                                                                      | ordered argument and, when needed, active-command patches                                        |
| `pi.tool_execution.started`      | `RuntimeCommandStatePort.startCommand(...)`                                                                                                                                                       | descriptor-derived command/surface read-model refresh only                                       |
| `pi.tool_execution.updated`      | `RuntimeCommandStatePort.recordCommandEvent(...)`                                                                                                                                                 | descriptor-derived command/surface read-model refresh only                                       |
| `pi.tool_execution.finished`     | `RuntimeCommandStatePort.finishCommand(...)`                                                                                                                                                      | descriptor-derived command/surface read-model refresh only                                       |
| `pi.assistant_message.committed` | `commitAssistantMessage(...)`, or `failAssistantMessage(...)` for `error`/`aborted`; commits full ordered content, usage/cost, response metadata, terminal status, timestamps, and Pi history ref | one `surface` invalidation and `assistant_message_finished`                                      |
| `pi.turn.finished`               | none; this is an internal Pi LLM/tool-loop turn boundary                                                                                                                                          | none; it never settles the Svvy prompt                                                           |
| `pi.agent.finished`              | finish the Svvy turn, settle the claimed queue row, and fail any still-active assistant before settlement                                                                                         | descriptor-derived invalidations and the terminal `prompt_status` patch                          |

Transcript mutations are authoritative even if renderer notification publication fails. A later
contiguous patch exposes the sequence gap, and surface reopen publishes a reset from the durable
cursor so consumers rebaseline all four surface read models. State compare-and-set failures stop the
event lane rather than applying a patch to a stale message or generation.
...
After pi stream completion or typed failure, runtime calls one claim-fenced
`RuntimeTurnStatePort.settlePromptTurn(...)` transaction that terminalizes named dangling commands,
settles the queue row to delivered/failed/cancelled, and records the exact completed/failed/cancelled
turn status. Interruption, forced shutdown, and startup recovery use the atomic
`RuntimeTurnStatePort.recoverInterruptedTurn(...)` transition. Runtime records separate recovery
work through `RuntimeRecoveryStatePort` only when retry or later repair is actually required.
...
| ------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| successful turn                       | `RuntimeTurnStatePort.settlePromptTurn({ status: "completed", ...claimFence })`; terminal command rows already finished by tool handlers                                                                                                       | `surface`, `sessionNavigation`, `commandInspector`, `requestInput` when affected             | final `surface.stream` turn-settled patch plus descriptor-derived `workspace_read_model.changed` events                                                     | No retry. An exact duplicate settlement is an invalidation-free no-op.                                                                                                   |
| pi adapter failure before first token | `RuntimeTurnStatePort.settlePromptTurn({ status: "failed", ...claimFence })`; dangling command ids are empty                                                                                                                                   | `surface`, `sessionNavigation`, `appLogs`                                                    | `surface.stream` error/rebaseline event; descriptor-derived read-model changed events                                                                       | Runtime records retryable recovery only when the failure reason is startup/session-reference recoverable; provider/auth/path failures remain terminal.                   |
| pi stream failure mid-turn            | `RuntimeTurnStatePort.settlePromptTurn({ status: "failed", terminalCommandIds, ...claimFence })` atomically fails open commands, queue, and turn                                                                                               | `surface`, `sessionNavigation`, `commandInspector`, `requestInput`, `appLogs`                | `surface.stream` interrupted/error patch; `command.changed`; descriptor-derived read-model changed events                                                   | Runtime does not replay partial pi streams; startup recovery uses the corresponding atomic interrupted-turn transition.                                                  |
...
| user abort                            | `RuntimeQueueStatePort.cancelSurfaceMessage(...)` for queued rows; active turn path interrupts the retained prompt fiber and uses `RuntimeTurnStatePort.settlePromptTurn({ status: "cancelled", ...claimFence })` or interrupted-turn recovery | `surface`, `sessionNavigation`, `commandInspector`, `requestInput`                           | `surface.stream` cancellation patch; `queue.changed`; `command.changed`                                                                                     | Runtime surface scope owns live interruption; the atomic state transition owns terminal facts.                                                                           |
| runtime shutdown                      | admission closes first; after bounded drain, runtime force-interrupts retained prompt fibers and calls `RuntimeTurnStatePort.recoverInterruptedTurn({ terminalStatus: "cancelled" })`, then cancels returned live command ids                  | `surface`, `sessionNavigation`, `commandInspector`, `appLogs` after recovery commits         | descriptor-derived read-model changed events; no new stream deltas after shutdown begins                                                                    | Startup recovery repeats the same idempotent transition for any surviving active turn before stale-claim normalization.                                                  |
| surface close                         | `RuntimeSurfaceLifecycleStatePort.closeSurface(...)`; active prompt cancellation uses retained-fiber interruption plus atomic prompt settlement/interrupted-turn recovery                                                                      | `surface`, `sessionNavigation`, `requestInput`, `commandInspector`                           | `surface.changed` with reason `"surface.closed"` plus `surface.stream` cancellation/rebaseline when an active turn existed                                  | Surface scope closes process-local waits/subscriptions; durable atomic recovery owns any still-active turn.                                                              |
...
type RuntimeFacadeShape = {
  ...
  generatedContext: RuntimeGeneratedContextFacade;
  extensions: RuntimeExtensionsFacade;
...
`sourceEdits` owns renderer/headless source open/save requests and workflow-agent lifecycle for
file-backed extension and Workflows source files. Runtime validates caller intent, asks
`@svvy/extensions` to resolve/read the editable file,
...
then, for workflow-agent source, parse canonical agent parameters and admit provider/model/reasoning
through the app-global `RuntimeLayerModelResolverPort` and `RuntimeLayerProviderAuthPort`, independent
of the caller workspace and `PiAdapter.models.list(...)`, before delegating the file-level compare-and-swap or overwrite to
...
lineage, and source path. Workflow-agent saves atomically commit the matching valid observation with
`RuntimeSourceStatePort.recordWorkflowAgentSourceSave(...)`; other source kinds use
`recordSourceSave(...)`. Runtime then publishes only the committed `afterCommit` read-model
invalidation descriptors. `@svvy/extensions` writes source
...
Workflow-agent create/duplicate/delete use the same file-owner/state-owner split. Runtime admits
live model/auth/reasoning before create or duplicate, delegates no workspace identity to Extensions,
atomically commits exact source-save plus valid-observation evidence or tombstone-delete evidence,
and schedules workflows invalidation. A
failed post-file commit or invalidation hint produces a durable source-owned `source_reconcile`
recovery item. Recovery reconstructs the latest workflow-agent observations from disk through the
same scan/admit/reconcile lane before package refresh; it never replays stale workflow-agent facts
through generic `recordSourceSave(...)` or `recordSourceDelete(...)`.
...
The package-private `RuntimeWorkflowAgentSourceIndex` owns the app-global workflow-agent scan/admit/
reconcile sequence. Startup calls `Extensions.sources.scaffoldMissingWorkflowAgents()` before the
initial scan, admits every structurally valid observation through exact model, provider-auth, and
reasoning checks, atomically commits one `reconcileWorkflowAgentSources(...)` batch, and publishes
that batch before readiness returns. Admission failures invalidate only their observation and append
a path-qualified diagnostic; structural or unreadable observations remain invalid without aborting
the scan. The aggregate fingerprint hashes canonical sorted observations and diagnostics and excludes
`observedAt`/`scannedAt`, so identical disk evidence has the same fingerprint across scans.

The package-private `RuntimeExtensionStartupReconcileService` owns fresh-install builtin readiness.
Before workflow-agent reconciliation, startup calls `Extensions.builtin.scaffoldMissing()`, commits
and publishes initial extension registry/build evidence, and builds only pristine builtin records
whose build is required and currently missing or stale. Builds are deterministic and sequential;
customized builtins and user extensions are never auto-built. Startup then re-observes and commits
final registry/build evidence, refreshes and commits CLI readiness, and fails closed unless every
pristine required builtin is current and context-ready.

Every app-global generated `@svvyx/workflows` refresh runs this same reconciliation once before core
contract materialization or package build. Any invalid observation blocks the build only after its
invalid row and committed descriptors have published, preserving the last ready generated package.
Scan, state-commit, or publication failure also stops before build. Refreshes that do not build
`@svvyx/workflows`, including workspace-link-only repair, do not perform this source-index pass.
...
Draft persistence, queued-message restore, and queue reorder validate the explicit `PromptTarget`,
commit through the corresponding core-owned state port, and publish only the returned post-commit
invalidations. Restore accepts only a decodable queued `user_message` owned by that target and
cancels only rows still in `queued` or `steering`; reorder validates both the moving row and optional
anchor row before committing.
...
`OpenExtensionSourceEditInput`, the runtime-specific save/create/duplicate/delete wrappers, their
extension-owned nested source inputs, and all source session/lifecycle results are core-owned
source-edit contracts reused by runtime exactly. Runtime wrappers carry `workspaceId` only where
live provider/model/reasoning admission requires workspace-scoped pi model/auth metadata; runtime
passes only each wrapper's nested `source` input to `@svvy/extensions`.
`RuntimeSourceEditsService` and `RuntimeSourceEditsFacade` are runtime-owned service/facade
group shapes, not `@svvy/core` exports. Runtime does not define a renderer-specific source shape. `open(...)` delegates the file read
...
For builtin/user extension sources, the shared extension source coordinator encloses the complete
open/CAS/write/source-fact/publication/invalidation-hint sequence. This prevents lifecycle tree
promotion, snapshot root restore, or global lifecycle-journal recovery from invalidating a successful
save. Workflow source kinds use their separate workflow source and recovery authority.
...
type RuntimeSaveExtensionSourceEditInput = {
  workspaceId: WorkspaceId;
  source: SaveExtensionSourceEditInput;
};
```
...
Exact workflow-agent lifecycle inputs/results:
...
type WorkflowAgentSourceLifecycleResult = {
  ...
  fileWriteReceipt: { path: AbsolutePath; previousExists: false; bytes: number };
...
type WorkflowAgentSourceDeleteResult = {
  ...
  fileWriteReceipt: { path: AbsolutePath; deleted: true };
  reconcileRequired: true;
};
```

`defaultAgent`, `explorerAgent`, `implementerAgent`, and `reviewerAgent` are reserved create targets
and non-deletable default sources. Create maps export name to canonical id, trims the nonblank
display name into label, and preserves optional extension order. Duplicate preserves the canonical
provider/model/reasoning, overrides, and order while revalidating copied extension references.
...
The implemented workflow-agent lifecycle is public as `sourceEdits.createWorkflowAgent`,
`sourceEdits.duplicateWorkflowAgent`, and `sourceEdits.deleteWorkflowAgent`. Each method admits the
file's exact provider/model/reasoning tuple through the app-global model resolver and provider-auth
ports before a create/duplicate/save write, delegates only the nested source input to
`Extensions.sources`, atomically records the exact save plus valid observation or exact deletion
tombstone through `RuntimeSourceStatePort`, publishes only committed descriptors, and hints the
workflows source invalidation lane. If the file write succeeds but source fact commit or invalidation
scheduling fails, runtime enqueues app-scoped `source_reconcile` recovery with source ownership. The
recovery worker treats the generic payload as lineage only for workflow agents: it invokes the
generated Workflows refresh lane, whose first step re-scans current disk state, re-applies admission,
and atomically reconciles observations and tombstones before any package build.

Other workflow source lifecycle methods such as `sourceEdits.createWorkflowPrompt`, `sourceEdits.deleteWorkflowPrompt`,
...
Per-extension build reconciliation is an app-global runtime lane with one canonical sequence:

1. observe the complete extension registry;
2. call `Extensions.builds.observeCurrent(...)` for that exact registry aggregate;
3. commit the registry observation and the complete source/build evidence batch through
   `RuntimeExtensionStatePort`;
4. publish only the returned committed app `extensions` invalidation descriptors.

Startup, deterministic manual reconciliation, and watcher-triggered reconciliation use this same
sequence. If the registry commit succeeds and build-evidence commit fails, read-model fingerprint
gating makes the unmatched evidence unusable and the lane retries the same deterministic
observation; runtime never joins the old ready row to the new registry. A source fingerprint change
makes an existing required build stale immediately. Ordinary file edits stop there and do not
auto-build. Product actions whose Extension Managing contract requires an automatic build enqueue
the same explicit build lane after their source commit.

Per-extension current-build evidence, aggregate generated-context cache evidence, and
`RuntimeGeneratedPackageStatePort` facts for `@svvyx/extensions`/`@svvyx/workflows` are three
separate identities and lifecycles. Aggregate package success may consume current per-extension
evidence but cannot create it, make a stale build current, or satisfy extension runtime readiness.
...
- `RuntimeEventBus` is constructed inside `Runtime.layer`. App/bootstrap must not provide a
  ...
  `surface.stream` and `surface.changed` events before handing them to `RuntimeEventBus`. Durable
  transcript stream cursor authority belongs to `RuntimeTranscriptStatePort`; the publisher
  serializes publication, validates explicit committed sequences, and mirrors the latest published
  target/generation cursor so gaps are observable:
  ...
  The publisher stores one in-memory publication cursor per durable `surfacePiSessionId`. Each
  cursor records the current `streamGenerationId` plus the latest sequence published for that
  generation. Transcript-bearing callers first advance the durable state cursor and pass the
  committed `streamSequence` explicitly. The publisher accepts an initial explicit sequence when
  no in-memory cursor exists, then requires every later explicit sequence to be contiguous for the
  same generation; a generation change must begin at `1`. This mirror never becomes product truth.
  Package-private transitional callers may omit `streamSequence`, in which case the publisher
  allocates the next in-memory sequence without writing product state.

  `resetSurfaceStream(...)` publishes a `stream_reset` patch under the
  `streamGenerationId` passed by the caller. Surface reopen first advances the durable cursor and
  supplies both that committed reset event sequence and the prior committed
  `latestStreamSequence`. When omitted for a non-transcript caller, the publisher derives both
  values from its in-memory publication cursor. `RuntimeEventBus` remains the sole owner of app-wide
...
| Extension build service                       | `@svvy/runtime` package-private `RuntimeExtensionBuildService` / `layerRuntimeExtensionBuildService`                             | app-scoped map of one-permit lanes keyed by extension id; durable request-bound registry/source/build-attempt authority remains in state                                                                                                                                    | `layer-acquired`                                                         | `Runtime.layer` acquisition                                                                                                                                              | app runtime shutdown/restart drops only process-local lanes; request replay survives                                                  | yes within app runtime | same-id calls serialize; identical request ids resume/return stored outcomes; after attempt start every failure/interruption records one terminal receipt unless a terminal receipt already committed; permit releases on every exit                                        | success/failure/running/conflict replay, exact classification, start-publication failure terminalization, same-id serialization tests  |
| Extension startup reconcile service           | `@svvy/runtime` package-private `RuntimeExtensionStartupReconcileService` / `layerRuntimeExtensionStartupReconcileService`       | app-scoped startup orchestration over builtin source scaffolding, registry/build/readiness state commits, extension builds, and after-commit runtime-event publication                                                                                                      | `layer-acquired`                                                         | `Runtime.layer` acquisition; startup readiness invokes its one reconciliation effect                                                                                     | app runtime shutdown/restart; the service owns no durable handle beyond committed state                                               | yes within app runtime | interruption fails startup readiness and cancels remaining startup work; already committed source/build/readiness facts remain authoritative, and no success receipt is returned until required pristine builtins are context-ready                                         | fresh-install scaffold/build/readiness, existing/customized source preservation, typed failure, and publication tests                 |
| Extension source coordinator                  | `@svvy/runtime` package-private `RuntimeExtensionSourceCoordinator` / `layerRuntimeExtensionSourceCoordinator`                   | one app-scoped one-permit `Semaphore` exposed only through generic `serialized(effect)` policy; the permit encloses extension-source filesystem mutation plus nested reconcile, finalize, build, app-log, and publication work                                              | `layer-acquired`                                                         | `Runtime.layer` acquisition                                                                                                                                              | app runtime shutdown/restart releases the process-local permit; committed source/build/state facts remain authoritative               | yes within app runtime | interruption releases the permit on every exit; no second extension-source filesystem transaction enters while the active transaction is reconciling, finalizing, building, or publishing                                                                                   | cross-operation and cross-service extension-source serialization tests                                                                 |
| Extension lifecycle service                   | `@svvy/runtime` package-private `RuntimeExtensionLifecycleService` / `layerRuntimeExtensionLifecycleService`                     | app-scoped registry lock and per-extension one-permit mutation lanes nested inside the shared extension-source coordinator transaction over lifecycle source operations, build refresh, invalidation, app logs, and runtime events                                          | `layer-acquired`                                                         | `Runtime.layer` acquisition                                                                                                                                              | app runtime shutdown/restart drops process-local lanes; committed source/build/state facts remain authoritative                       | yes within app runtime | interruption releases the per-id and app-global permits; incomplete filesystem operations follow extension atomic staging policy and no success invalidation is published without the required committed/reconciled outcome                                                 | create/duplicate/delete/reset/instruction/TypeScript-API mutation, per-id and cross-id serialization, invalidation, and failure tests  |
| Extension snapshot service                    | `@svvy/runtime` package-private `RuntimeExtensionSnapshotService` / `layerRuntimeExtensionSnapshotService`                       | one app-scoped one-permit orchestration lane over durable snapshot/settings/restore state, payload and secret stores, extension source restore, build refresh, invalidation, and context-impact facts                                                                       | `layer-acquired`                                                         | `Runtime.layer` acquisition                                                                                                                                              | app runtime shutdown/restart releases the process-local lane and host handles; durable restore/cleanup attempts remain recoverable    | yes within app runtime | interruption releases the lane and leaves the exact durable restore phase recoverable; raw secret values remain confined to the host secret-values port                                                                                                                     | save/load/rename/delete, phase recovery, replay, cleanup, blocked build, and secret redaction tests                                    |
| Generated-context binding service             | `@svvy/runtime` package-private `RuntimeGeneratedContextBindingService` / `layerRuntimeGeneratedContextBindingService`           | transient assembly coordinator over state-owned actor bindings/external usage, extension registry/current builds, file-backed contributors, request-input settings, crypto digesting, and atomic state binding                                                              | `layer-acquired`                                                         | `Runtime.layer` acquisition                                                                                                                                              | app runtime shutdown/restart; generated prompt bindings remain durable in state                                                       | yes within app runtime | interruption cancels only the active observation/read/assembly before atomic binding, or observes the fully committed binding after commit; no catalog/cache fallback is admitted                                                                                           | initially unbound first dispatch, atomic bind/reopen, refresh ordering, readiness, and failure mapping tests                           |
| Generated-context preview service             | `@svvy/runtime` package-private `RuntimeGeneratedContextPreviewService` / `layerRuntimeGeneratedContextPreviewService`           | transient read-only query composition over state-owned preview subjects and external-instruction usage, extension registry/current-build observations, and file-backed contributor reads                                                                                    | `layer-acquired`                                                         | `Runtime.layer` acquisition                                                                                                                                              | app runtime shutdown/restart                                                                                                          | yes within app runtime | interruption cancels only the active read/assembly query; no source, state, build, probe, or materialization mutation is admitted                                                                                                                                           | orchestrator/handler/workflow preview, ordering/bypass, usage precedence, readiness failure, exact facade codec, and no-mutation tests |
| Surface stream cursor lane                    | `@svvy/runtime` `RuntimeSurfaceEventPublisher`; durable cursor authority: `@svvy/state` through `RuntimeTranscriptStatePort`     | target-local publication-cursor mirror keyed by durable `surfacePiSessionId`, active `streamGenerationId`, and latest published `streamSequence`, plus one runtime-owned one-permit `Semaphore` lane; transcript callers supply the cursor committed in state               | `layer-acquired`                                                         | `Runtime.layer` acquisition                                                                                                                                              | app runtime shutdown/restart drops only the publication mirror                                                                        | yes within runtime     | automatic permit release on publication success, failure, interruption, or shutdown; durable transcript/read-model cursor remains authoritative                                                                                                                             | explicit committed sequence, gap rejection, reset, failure propagation, and permit-release tests                                       |
...
| Surface runtime scope service                 | `@svvy/runtime` surface scope service                                                                                            | runtime-owned keyed child scope plus retained pi session and active-prompt fiber                                                                                                                                                                                            | `keyedOwnerScoped`                                                       | `surfaces.open(...)`, turn delivery, wait/command retention                                                                                                              | zero-retain release after the active prompt clears, invalidation, idle TTL, or app-layer scope close                                  | yes                    | release during an active prompt records zero retainers and closes after prompt finalization; layer finalization force-interrupts and joins active prompts, then closes every remaining pi session and child scope                                                           | surface acquired/released/finalized, active-release, joined-interruption, and layer-scope cleanup tests                                |
...
| Queue wake broker                             | `@svvy/runtime` `RuntimeQueueWakeBroker` / `layerRuntimeQueueWakeBroker`                                                         | one app-scoped registration slot that decouples semantic post-commit wake requests from the queue dispatcher lifecycle                                                                                                                                                      | `layer-acquired`                                                         | `Runtime.layer` acquisition                                                                                                                                              | app runtime shutdown/restart                                                                                                          | yes                    | wake fails closed until the dispatcher registers; registration cleanup cannot remove a newer dispatcher                                                                                                                                                                     | unavailable-before-registration, registered forwarding, stale-cleanup, and layer-composition tests                                     |
...
| Source reconcile recovery worker              | `@svvy/runtime` `RuntimeSourceReconcileRecoveryWorker`                                                                           | one app-scoped dropping wake queue, worker and cadence fibers, plus fenced durable `source_reconcile` claims                                                                                                                                                                | `layer-acquired`                                                         | `Runtime.layer` acquisition; durable source-reconcile recovery row commits wake the worker                                                                               | app runtime scope close shuts the queue, interrupts worker fibers, and releases an active claim through the exact durable lease fence | yes within runtime     | non-workflow sources replay their exact generic fact then reconcile; workflow-agent rows run latest-disk scan/admit/atomic reconcile through generated refresh; row failures use configured exponential retry; scope interruption returns the exact fenced claim            | generic source replay, workflow-agent latest-source recovery, retry timing, owner validation, and scope-release tests                  |
...
| RuntimeExternalInstructionScanInputPort       | `@svvy/runtime` `RuntimeExternalInstructionScanInputPort`                                                                        | public bootstrap composition tag that resolves workspace identity, canonical cwd/root, home directory, and state-owned external-instruction root configuration without exposing paths to renderer consumers or accepting legacy path-keyed usage controls                   | `layer-acquired`                                                         | `Runtime.layer` acquisition through app/bootstrap workspace registry binding                                                                                             | app runtime shutdown/restart                                                                                                          | yes                    | missing workspace authority fails with typed `RuntimeContractError`; projection state remains unchanged                                                                                                                                                                     | startup/manual/watcher scan coordination, workspace scoping, and legacy-control exclusion tests                                        |
...
| Runtime shutdown admission service            | `@svvy/runtime` package-private `RuntimeShutdownAdmission` / `layerRuntimeShutdownAdmission`                                     | one app-scoped short `Semaphore` bookkeeping lane, concurrent admitted-work count/drain `Deferred`, shutdown-started marker, and single-use shared shutdown outcome `Deferred`                                                                                              | `layer-acquired`                                                         | `Runtime.layer` acquisition                                                                                                                                              | app runtime shutdown/restart                                                                                                          | yes within runtime     | admitted work stays concurrent and interruptible; marker leadership, count release, and receipt completion are interruption-safe; shutdown awaits the tracked admitted set before its interruptible bounded drain; concurrent/repeated shutdown callers observe one outcome | concurrent admission, admitted-work drain, shutdown coalescing, and late accepted-tool rejection tests                                 |
...
The extension source coordinator is one shared layer instance used by lifecycle and TypeScript-API
mutations, builtin/user `sourceEdits.save(...)`, snapshot source capture/load/recovery, and
extension-domain source-reconcile recovery entry points. Lifecycle and snapshot code call source
invalidation while already holding that permit, so those nested calls use the internal service
directly and do not reacquire it. Public `sourceInvalidation.reconcile(...)` and
`applyCommittedScanEvent(...)` facade entries, plus the durable source-reconcile worker, acquire the
permit before any extension lifecycle journal recovery. Workflow source operations stay outside
this coordinator because they do not access extension source roots.
...
and event-bus acquisition have completed or failed startup. Initial app source reconciliation first
scaffolds missing builtin sources, commits initial registry/build evidence, builds missing or stale
pristine required builtins, commits final registry/build/CLI-readiness evidence, and proves those
builtins context-ready. It then scaffolds missing packaged workflow-agent defaults and
commits/publishes the deterministic admitted workflow-agent index. A missing template, required
builtin build/readiness failure, scan failure, state failure, or publication failure is a typed
`app-source-reconcile` startup failure. Generated-package builds remain separate refresh work and
are not a substitute for this source-index readiness gate. The runtime-owned readiness service must bound this wait by
...
| App source reconcile                    | app           | startup readiness receipt        | runtime facade exposure and generated import claims              | missing builtin sources scaffolded; initial and final registry/build evidence committed; missing/stale pristine required builtins built and context-ready; CLI readiness committed; missing workflow-agent defaults scaffolded; admitted source/index rows plus app-global fingerprint committed; descriptor publication accepted | invalid individual workflow-agent observations are committed with diagnostics; customized builtin and user sources remain untouched | packaged template, required builtin build/readiness, scan, state commit, or publication failure |
...
- Runtime owns an internal surface queue dispatcher service:
  ...
  `queue-steered -> steer`, `runtime-queue-inserted -> enqueue`, and
  `startup-recovery -> recovery`. Recovery, `retry_due`, and `lease_released` hints may be emitted
  only by runtime recovery and lease workers after reading
...
Runtime validates the signed subprocess result and decodes only the exact core schema-backed
response-bearing request intent for the app-owned namespace. Extension Managing requests enter the
same `runtime.extensions.*` and `runtime.sourceEdits.configureTypescriptApi(...)` methods used by
other app consumers; the parent returns the committed result and authoritative command facts to the
owning Shell command session. The child never applies source, usage, snapshot, build, context-impact,
state, or notification work. There is no `SvvyxRuntimeEffectTransportRequest`, no
`runtime_effect.request` child intent, and no child-to-parent `RuntimeEffectRequest` replay path.
Adding another response-bearing app-owned request kind requires a core-owned schema, runtime-owned
apply path, package-boundary tests, and source-of-truth spec updates before it can be accepted.
...
That subpath adapts an existing runtime instance to the app edge only for direct-tool launch,
approval, and extension loading. Accepted `request_user_input` execution stays wholly inside the
runtime prompt executor through `RuntimeAcceptedNativeToolExecution`; no app/bootstrap adapter or
Bun-native tool wrapper may invoke it. The subpath does not create a runtime, hold its own singleton
services, expose the service tag, expose the service interface, expose the service layer, call
extension handlers directly from app code, expose state command ports, or add a desktop/headless
facade group.
...
`setVariant(...)` and `setBlockingTimeout(...)` use the named strict codecs from `@svvy/core`.
Both methods commit through `RuntimeRequestStatePort` and publish the returned `afterCommit`
descriptors before returning the committed `RequestInputSettings`. After variant publication,
runtime snapshots `RuntimeWorkspaceScopeService` and refreshes generated context once for every
currently acquired workspace with reason `profile-settings-changed`; timeout mutation performs no
generated-context refresh because timeout policy does not change prompt text or tool declarations.
Generated-context fingerprints and native-tool schema descriptions depend on the committed mode
only. Runtime prompt preparation reads committed request-input settings and passes the active mode
as `requestInputVariant` to `Extensions.nativeTools.declarations(...)`.
...
request, and records the matching durable user wait in one state transaction, then schedules the
timeout when enabled and parks the accepted tool call on a surface-scoped live handoff. The
model-facing tool result is not returned until all questions have user answers or timeout defaults.
The wait is represented durably by request/command/turn facts; the process-local Deferred is only a
live handoff and is recreated or completed by recovery.

Every accepted `request_user_input` invocation enters
`RuntimeAcceptedNativeToolExecution.runRequestUserInput(...)`. At the beginning of that invocation,
runtime reads `RuntimeRequestStatePort.readRequestInputSettings()` exactly once and freezes the
returned snapshot. That one snapshot supplies request creation mode/timeout policy, command progress
variant facts, and the blocking-versus-nonblocking branch. The extension handler intent contains no
mode or timeout fields. Blocking results bypass the generic accepted-tool command finisher because
the request-input wait lifecycle owns waiting and terminal command settlement; nonblocking command
settlement remains inside the accepted request-input operation.
...
Blocking request-input terminal request/answer resolution is one state compare-and-set transition
keyed by `requestId`, owning command id, current request status, and timer version/deadline. The same
transaction commits the terminal request/answer facts, terminalizes the linked command, and clears
only the matching user wait. User answers, live timeout fibers, the timeout scan worker, request
cancellation, turn interruption, surface/workspace close recovery, and startup recovery use that
same transition. The first terminal commit wins. Losing contenders observe `stale-state`, release
only their process-local resources, and must not publish duplicate invalidations, resolve a
`Deferred`, insert answer queue rows, or settle the command. Completed/expired requests finish the
command as `succeeded`; cancelled requests finish it as `cancelled` except interrupted-turn recovery
uses the turn's chosen `failed`/`cancelled` terminal status. Only after the transaction's returned
invalidations publish may the live `Deferred` resolve or fail and its registry entry be removed.
Recovery lists only still-open blocking requests, reconstructs their live waits, and reforks only
their current unpaused timeout generation.
...
Runtime uses `DateTime.now` plus `DateTime.addDuration` and `DateTime.formatIso` only to create
persisted UTC instants such as `notBefore`, `nextAttemptAt`, `expiresAt`, and committed timestamps.
Runtime uses `Clock` / `Schedule` for
