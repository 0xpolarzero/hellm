...
| Extension lifecycle service                   | `@svvy/runtime` package-private `RuntimeExtensionLifecycleService` / `layerRuntimeExtensionLifecycleService`                     | app-scoped registry lock and per-extension one-permit mutation lanes over extension source operations, build refresh, invalidation, app logs, and runtime events                                                                                                            | `layer-acquired`                                                         | `Runtime.layer` acquisition                                                                                                                                              | app runtime shutdown/restart drops process-local lanes; committed source/build/state facts remain authoritative                       | yes within app runtime | interruption releases every acquired permit; incomplete filesystem operations follow extension atomic staging policy and no success invalidation is published without the required committed/reconciled outcome                                                             | create/duplicate/delete/reset/instruction mutation, per-id serialization, invalidation, and failure tests                              |
