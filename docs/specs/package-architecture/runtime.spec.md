...
scaffolds missing packaged workflow-agent defaults, then commits/publishes the deterministic admitted
workflow-agent index; a missing template, scan failure, state failure, or publication failure is a
typed `app-source-reconcile` startup failure. Generated-package builds remain separate refresh work
and are not a substitute for this source-index readiness gate. The runtime-owned readiness service must bound this wait by
...
| App source reconcile                    | app           | startup readiness receipt        | runtime facade exposure and generated import claims              | missing workflow-agent defaults scaffolded, admitted source/index rows plus app-global fingerprint committed, and descriptor publication accepted | invalid individual observations are committed with diagnostics; no build is claimed ready from those rows      | packaged template, scan, state commit, or publication failure                                       |
