...
- Caller: `@svvy/runtime` at the committed queue-to-turn dispatch boundary.
- Methods: readPolicy.
- Rule: resolves the workspace cwd from the addressed workspace record and the current approval mode
  from app-global durable preferences. It is read-only and cannot derive policy from renderer state,
  process cwd, or a source-checkout path.

The exact tool execution policy state port is:
...
| Runtime tool execution policy state port     | core-owned `RuntimeToolExecutionPolicyStatePort` implemented by the workspace state router                         | DB/product-state-backed | `layer-acquired` | `layerWorkspaceStateRouter` projects app-global approval preferences and the addressed workspace record                                                                                                                                                                                                                                                                     | app runtime disposal closes access handles only                                                                                                                                         | yes                                                                             | interruption cancels the current policy read only; committed preferences and workspace cwd remain authoritative      | workspace routing and prompt dispatch policy propagation tests                                |
