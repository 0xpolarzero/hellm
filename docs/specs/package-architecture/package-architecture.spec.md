...
| `createRuntimeModelResolverHostLayer(...)`       | `RuntimeLayerModelResolverPort`                          | `runtime.spec.md`                                 | exact app-global pi-ai registry lookup from provider/model ids to normalized identity plus supported reasoning levels                                                                                                            | provider auth, workspace model catalogs, model selection policy, prompt construction, implicit alternate-model selection, pi session ownership                              | app runtime layer                                | exact registry misses fail typed; runtime combines the narrow result with `RuntimeLayerProviderAuthPort` for workflow-agent admission                                                                                                                 | no lifecycle beyond model registry handle                                                                                | fake model resolver layer with exact valid/missing identities and supported/unsupported reasoning                                     | runtime methods receive only exact identity/reasoning admission evidence through the named port; no broad public model API                                    |
