...
edit open/save sessions, and workflow-agent create/duplicate/delete lifecycle. Generated context
...
`generated-instruction`, `svvyx-source`, and `command-schema`.
...
`svvyx` command source, read-only app-owned generated command schemas, plus Workflows
source-library files for `workflow-agent`, `workflow-prompt`, `workflow-component`, and
`workflow-workflow`. Virtual native-tool schemas and workspace-CWD/shared generated TypeScript
declaration views remain explicitly non-openable until they have authoritative app-owned records.
All
