...
import type { ExtensionCliRequirementProbePort } from "./extension-cli-requirement-probe-port";
import type { ExtensionBuildProcessPort } from "./extension-build-process-port";
...
The public `Extensions` service exposes registry reads, per-extension source/build observation and
build execution, atomic extension and editable-instruction source lifecycle, snapshot source capture
and journaled source-restore preparation/application, CLI requirement readiness probing through closed package-local host ports, actor binding and visible-record helpers,
native tool declaration/metadata/handler lookup, generated-package refresh/link planning, source
edit open/save sessions, validated atomic TypeScript-API source configuration, and workflow-agent
create/duplicate/delete lifecycle. Generated context
building, `svvyx` dispatch, execute-typescript facade declaration generation, env/dependency
readiness, builtin scaffold/reset, external-instruction scanning, and prompt/component/workflow
source lifecycle remain package responsibilities exposed only through package-root contracts named
by this spec and the package-boundary ledger; they are not additional public `Extensions` service
methods unless this spec names the exact method shape.

`Extensions.snapshots.captureSourcePayload(...)` walks only materialized `sources/user` and
`sources/builtin` roots. It accepts exact actor/profile usage, non-secret overrides, and secret
presence identities from state-owned callers without reading or writing state and never receives
secret values. Capture rejects symlinks, escaped/non-canonical paths, non-regular files, missing or
mismatched manifests, oversized files/payloads, and excludes `.svvy`, builds, generated output,
dependency/package-manager directories, and package files. Every captured file carries canonical
POSIX-relative path, byte size, base64 bytes, and SHA-256.

`Extensions.snapshots.prepareSourceRestore(...)` validates the complete payload and stages only
contained source roots under the app-owned extension root. `applySourceRestore(...)` revalidates the
plan, payload digest, staged bytes, hashes, and manifests before journaled rename promotion of both
builtin and user roots. Failure or interruption rolls back prior roots; a later service instance can
recover the durable journal/plan and retry. Results expose opaque plan ids/counts only, never host
paths. These methods perform no state, secret-store, runtime, generated-package, CLI, or UI writes.
Capture and prepare both enforce the same per-file and aggregate byte limits plus closed source/file
count caps before base64 decode or staging writes, reject duplicate ids across categories, and require
canonical category/id/path ordering. After orchestration no longer needs a completed plan,
`finalizeSourceRestore(...)` is the only cleanup owner: it removes the contained package-owned plan
directory idempotently and refuses cleanup while an active restore journal exists. Runtime never
deletes `.svvy/snapshot-restore` paths directly.
...
`GeneratedPackageRootPort`, `WorkspaceSourceLinkPort`, and
`ExtensionCliRequirementProbePort` as a layer requirement. Public
...
The exact CLI probe primitive is:

```ts
interface ExtensionCliRequirementProbePortService {
  probe(
    plan: ExtensionCliRequirementProbePlan,
  ): Effect.Effect<ExtensionCliRequirementProbeEvidence, ExtensionError>;
}

interface ExtensionCliRequirementProbePort {
  readonly _tag: "ExtensionCliRequirementProbePort";
}

const ExtensionCliRequirementProbePort = Context.Service<
  ExtensionCliRequirementProbePort,
  ExtensionCliRequirementProbePortService
>("@svvy/extensions/ExtensionCliRequirementProbePort");
```

The build process primitive is similarly closed: `ExtensionBuildProcessPort.run(plan)` receives
only an app-owned contained source root, one operation-scoped contained staging root, contained
generator script/output paths, one optional contained source-svvyx runtime bundle request, explicit
argv, expected process-output identities, a timeout, and stdout/stderr caps. It receives no env map,
ambient-env flag, state port, runtime command identity,
approval authority, dependency-install request, or current-build path. It returns bounded terminal
evidence plus process-owned staged file evidence and a strictly decoded svvyx command manifest when
the runtime request is present. It never promotes `current/` or writes product state. Runtime bundling
and manifest extraction run in a direct app-owned child with an empty environment and bounded
protocol; arbitrary extension code is never imported into the main app process.

`Extensions.builds.build(...)` accepts one registry-known extension and its canonical source
observation, rereads and verifies the source fingerprint, validates the process evidence against
the staged files, writes static source outputs plus package-owned command manifests and TypeScript
declarations, derives output/context/build fingerprints, writes the evidence manifest, and
atomically promotes staging to `current/`. Identity mismatch, stale source, unexpected/missing
output, nonzero exit, timeout, helper failure, or evidence mismatch fails with `ExtensionError` and
leaves the prior current build untouched.

Registry observations declare `svvyxImplementation` explicitly. User/editable svvyx sources use
`source-runtime` with `source/index.ts`. App-native `extension-managing`, `artifacts`, and `workflows`
records use package-owned command metadata and carry its derived SHA-256 fingerprint; they require no
editable source or runtime bundle. Build recomputes that fingerprint before writing metadata outputs.

`ExtensionCliRequirementProbePlan` is immutable data with probe kind, direct executable, argv,
explicit non-secret env, `extendEnv: false`, timeout, and stdout/stderr caps. It has no shell text,
working-directory authority, ambient env inheritance, secret references, install/update command,
build request, or runtime command identity. Package-runner declarations use `resolve-executable`
and empty argv so readiness inspection never fetches or executes a package. The probe port returns
only bounded typed evidence; `Extensions.dependencies.refreshReadiness(...)` interprets that
evidence into deterministic readiness and never writes state.

### Per-extension source and build evidence

Per-extension evidence uses the core-owned schemas and is independent from generated-package
evidence. The registry declaration carries `buildRequirement: "required" | "not-required"`; the
build service never infers it from category or interface alone. The valid matrix is:

| Registry/source kind                                                                             | `buildRequirement` | Per-extension current build                             |
| ------------------------------------------------------------------------------------------------ | ------------------ | ------------------------------------------------------- |
| builtin or user source with generated instruction, declaration, command, or runtime contributors | `required`         | required and fingerprint-gated                          |
| builtin or user source consumed directly with no generated contributor                           | `not-required`     | `not-required`; no current manifest                     |
| missing/unmaterialized required builtin source                                                   | `required`         | `missing`; materialize before building                  |
| removed registry record                                                                          | absent             | no usable evidence; reconciliation prunes its state row |

Each normal registry observation also carries the package-owned `usagePolicy`. Builtin canonical
order is the exhaustive `BUILTIN_EXTENSIONS` order; user extensions follow builtins in canonical
extension-id order. Builtin baseline states are the exact defaults in the builtin inventory. User
baseline states are orchestrator `loaded`, handler `unavailable`, and workflow-task `loaded`.
Only `web` declares `networkAccess: "required"`; observation never reads the current network
setting. Only `extension-loading` is non-configurable, with fixed reason
`Extension Loading is fixed always-loaded so actors can inspect and load available extensions.`;
all other normal records are configurable with `fixedReason: null`. External instructions remain
outside this normal-extension policy contract.

An external instruction or direct-source record cannot declare `required`. A local record that
declares any generator, generated instruction, TypeScript declaration, generated command manifest,
or runtime compilation contributor cannot declare `not-required`. Interface kind alone does not
create an exemption.

`ExtensionSourceFingerprint` is the lowercase `sha256:<64-hex>` digest in the
`svvy-extension-source-v1` domain. For one materialized required extension, the canonical input set
contains the exact bytes of canonical `manifest.json`, editable minimal instruction source,
configured non-generated full instruction sources, every referenced generator script, and every
regular file recursively under editable `source/`. Entries are normalized to contained POSIX
relative paths, assigned a stable role, sorted by `(role, relativePath)`, content-hashed with
SHA-256, length-framed as `(role, relativePath, contentHash)`, prefixed with the domain string, and
hashed again. A path escape, duplicate normalized path, unreadable declared input, or symlink whose
contained regular-file target cannot be proven makes the source observation `invalid`.

The input set excludes generated instruction outputs, generated TypeScript declarations, generated
command schemas, `current/` and `staging/` builds, aggregate context caches, generated package roots,
`package/node_modules`, `package/bun.lock`, install artifacts, trash, snapshots, env values and
secret references, CLI probe results, dependency approval/readiness, profile usage, actor bindings,
and `@svvyx/*` generated-package facts. Generated outputs are excluded so a build cannot change its
own source fingerprint.

`observeCurrent(...)` rereads the registry-named filesystem inputs and current manifest, validates
all manifest identities and file evidence, and returns one canonical extension-id-sorted batch tied
to the supplied registry aggregate fingerprint. For a required record, `buildRequired` is false
only when the source is materialized and valid, the current manifest decodes, its `extensionId` and
`sourceFingerprint` equal the observation, every listed output is contained and content-valid, and
the recomputed output fingerprint matches. Missing, stale, or invalid current evidence is never
ready. For a not-required record `buildRequired` is false and current status is `not-required`.

`ExtensionBuildId` is
`extension-build:<extension-id>:<output-sha256-hex>` and the output digest uses the same sorted,
length-framed algorithm over generated file evidence, excluding the evidence manifest itself.
`contextFingerprint` hashes only actor-facing instruction, command-schema, and declaration outputs;
runtime modules do not enter that digest.

The current manifest contains source/context/output evidence only. It never contains
`runtimeReady`, env values or status, CLI status, dependency approval/readiness, actor bindings,
aggregate generated-context evidence, or generated-package status. `contextReady` means only that
the validated actor-facing output exists. Runtime derives runtime readiness by joining that fact
with current CLI, env, and dependency facts.
...
| ------------------------------------------- | ------------------------------------------------ | ----------------------------------------------------- | ----------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------- |
| `builds.observeCurrent`                     | `Extensions.builds`                              | `ObserveExtensionSourceBuildsInputSchema`             | `ObserveExtensionSourceBuildsResultSchema`            | source-root and filesystem/path/crypto ports                                                                                                                                              | canonical editable source inputs and per-extension `current/` evidence only                                                                                                                                                                                                                                                                                                                                                                                                                                                                | no                                                                                                                                                                                                                                                         | no              | no                                                                                                                                                                                  | canonical input exclusions, category matrix, containment, stale/invalid/missing/not-required evidence, deterministic ordering                   |
| `builds.build`                              | `Extensions.builds`                              | `BuildExtensionInputSchema`                           | `BuildExtensionResultSchema`                          | source-root, filesystem/path/crypto, and closed `ExtensionBuildProcessPort`                                                                                                               | one registry-known canonical source observation, one operation-scoped staging root, and validated current manifest evidence                                                                                                                                                                                                                                                                                                                                                                                                                | yes; staging plus atomic current promotion only                                                                                                                                                                                                            | no              | no                                                                                                                                                                                  | identity/source CAS, contained env-free process plan, output evidence validation, failed-build preservation                                     |
| `sources.createExtension`                   | `Extensions.sources`                             | `CreateExtensionSourceInputSchema`                    | `CreateExtensionSourceResultSchema`                   | source-root, packaged-template, filesystem/path/crypto ports                                                                                                                              | app-global user extension source root; complete sibling staging tree promoted without clobber                                                                                                                                                                                                                                                                                                                                                                                                                                              | yes; one validated staged tree and atomic publication                                                                                                                                                                                                      | no              | no                                                                                                                                                                                  | strict kebab id and reserved-name rejection, instructions/svvyx only, TypeScript API invariant, races, cleanup, no partial live tree            |
| `sources.duplicateExtension`                | `Extensions.sources`                             | `DuplicateExtensionSourceInputSchema`                 | `DuplicateExtensionSourceResultSchema`                | source-root, packaged-template, filesystem/path/crypto ports                                                                                                                              | registry-known editable builtin/user source copied without generated output, mutation journals, installed dependencies, secrets, or app-owned native implementation                                                                                                                                                                                                                                                                                                                                                                        | yes; one validated staged tree and atomic publication                                                                                                                                                                                                      | no              | no                                                                                                                                                                                  | native/external rejection, current source validation, identity rewrite, excluded assets, races, cleanup                                         |
| `sources.deleteExtension`                   | `Extensions.sources`                             | `DeleteExtensionSourceInputSchema`                    | `DeleteExtensionSourceResultSchema`                   | source-root, filesystem/path/crypto ports                                                                                                                                                 | user source renamed to app-managed mutation trash plus recoverable prepared/committed mutation journal                                                                                                                                                                                                                                                                                                                                                                                                                                     | yes; atomic source-root rename and journal recovery                                                                                                                                                                                                        | no              | no                                                                                                                                                                                  | user-only policy, builtin/external rejection, containment, interrupted journal recovery                                                         |
| `sources.resetExtensionInstructions`        | `Extensions.sources`                             | `ResetExtensionInstructionsInputSchema`               | `ResetExtensionInstructionsResultSchema`              | source-root, packaged-template, filesystem/path/crypto ports                                                                                                                              | builtin instruction source set reset from packaged templates while preserving manifest fields and native/runtime implementation outside the instruction scope                                                                                                                                                                                                                                                                                                                                                                              | yes; whole-tree staging and rollback-capable atomic promotion                                                                                                                                                                                              | no              | no                                                                                                                                                                                  | builtin-only policy, exact instruction scope, unchanged no-op, native implementation preservation, rollback                                     |
| `sources.addInstruction`                    | `Extensions.sources`                             | `AddExtensionInstructionInputSchema`                  | `AddExtensionInstructionResultSchema`                 | source-root, packaged-template, filesystem/path/crypto ports                                                                                                                              | editable builtin/user `.mdx` contributor plus its manifest declaration; generated `.generated.md` output is read-only and never accepted by lifecycle DTOs                                                                                                                                                                                                                                                                                                                                                                                 | yes; whole-tree staging and rollback-capable atomic promotion                                                                                                                                                                                              | no              | no                                                                                                                                                                                  | strict basename, case-insensitive uniqueness, generated/external rejection, manifest/file atomicity                                             |
| `sources.removeInstruction`                 | `Extensions.sources`                             | `RemoveExtensionInstructionInputSchema`               | `RemoveExtensionInstructionResultSchema`              | source-root, packaged-template, filesystem/path/crypto ports                                                                                                                              | editable builtin/user `.mdx` contributor removed through mutation trash together with its manifest declaration; generated `.generated.md` output is read-only and never accepted by lifecycle DTOs                                                                                                                                                                                                                                                                                                                                         | yes; whole-tree staging and rollback-capable atomic promotion                                                                                                                                                                                              | no              | no                                                                                                                                                                                  | strict basename, missing/generated/external rejection, manifest/file atomicity, rollback                                                        |
| `sources.configureInstruction`              | `Extensions.sources`                             | `ConfigureExtensionInstructionInputSchema`            | `ConfigureExtensionInstructionResultSchema`           | source-root, packaged-template, filesystem/path/crypto ports                                                                                                                              | editable builtin/user instruction manifest bypass state                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    | yes only when bypass state changes; whole-tree staging and rollback-capable atomic promotion                                                                                                                                                               | no              | no                                                                                                                                                                                  | exact idempotence before builtin materialization, generated configurability, external rejection, rollback                                       |
...
| `sources.saveEditSession`                   | `Extensions.sources`                             | `SaveExtensionSourceEditInputSchema`                  | `SourceEditSaveResultSchema`                          | `FileSystem.FileSystem`, `Path.Path`, `Crypto.Crypto`, source-root ports                                                                                                                  | file-backed source bytes; workflow-agent saves require canonical task-agent JSON whose id matches its filename and whose specifically referenced non-builtin extension sources have current eligible manifests; runtime admits CAS and live model/auth/reasoning before calling this method, and writes state facts later through `RuntimeSourceStatePort`                                                                                                                                                                                 | yes; atomic replacement only after semantic validation                                                                                                                                                                                                     | no              | no                                                                                                                                                                                  | admitted CAS file write, workflow-agent reference validation, atomic write, file-write/state-fail recovery handoff                              |
| `sources.createWorkflowAgent`               | `Extensions.sources`                             | `CreateWorkflowAgentSourceInputSchema`                | `WorkflowAgentSourceLifecycleResultSchema`            | `FileSystem.FileSystem`, `Path.Path`, `Crypto.Crypto`, extension/workflow-agent source roots                                                                                              | canonical file-backed `~/.config/svvy/workflows/agents/*.agent.json`; export name becomes id, trimmed nonblank display name becomes label, explicit extension order is preserved, and only specifically referenced eligible user-extension manifests are inspected; state stores fingerprint/index facts later                                                                                                                                                                                                                             | yes; same-directory exclusive temp write followed by a no-clobber hard-link publication                                                                                                                                                                    | no              | no                                                                                                                                                                                  | reserved-name rejection, blank-label rejection, unknown/ineligible reference rejection, duplicate/racing source rejection, source fact evidence |
| `sources.duplicateWorkflowAgent`            | `Extensions.sources`                             | `DuplicateWorkflowAgentSourceInputSchema`             | `WorkflowAgentSourceLifecycleResultSchema`            | `FileSystem.FileSystem`, `Path.Path`, `Crypto.Crypto`, extension/workflow-agent source roots                                                                                              | canonical file-backed workflow-agent source; copied extension overrides and explicit order are revalidated against current specifically referenced extension manifests, label patches are trimmed/nonblank, and state stores fingerprint/index facts later                                                                                                                                                                                                                                                                                 | yes; same no-clobber publication as create                                                                                                                                                                                                                 | no              | no                                                                                                                                                                                  | duplicate actor source, current reference revalidation, exact extension override/order preservation, no clobber                                 |
| `sources.deleteWorkflowAgent`               | `Extensions.sources`                             | `DeleteWorkflowAgentSourceInputSchema`                | `WorkflowAgentSourceDeleteResultSchema`               | `FileSystem.FileSystem`, `Path.Path`, `Crypto.Crypto`, workflow-agent source root                                                                                                         | file-backed workflow-agent source; exact current source version is checked before deletion and state stores path/version/fingerprint tombstone facts later                                                                                                                                                                                                                                                                                                                                                                                 | yes                                                                                                                                                                                                                                                        | no              | no                                                                                                                                                                                  | reject default agent delete, stale source delete, recovery after file/state mismatch                                                            |
...
| `dependencies.refreshReadiness`             | `Extensions.dependencies`                        | `RefreshExtensionCliRequirementReadinessInputSchema`  | `RefreshExtensionCliRequirementReadinessResultSchema` | `Extensions.layer` closes over `ExtensionCliRequirementProbePort`; plans contain only direct executable/argv, explicit non-secret env with `extendEnv: false`, timeout, and output caps   | schema-backed registry CLI declarations plus operation-scoped host probe evidence; no DB reads/writes, package installs, builds, Shell commands, or managed secret values                                                                                                                                                                                                                                                                                                                                                                  | no                                                                                                                                                                                                                                                         | no              | no                                                                                                                                                                                  | exact/unversioned/update/mismatch/missing readiness, package-runner no-fetch, shell-control rejection, no ambient env                           |
| `builtin.scaffoldMissing`                   | `Extensions.builtin`                             | none                                                   | `ScaffoldMissingBuiltinSourcesResult`                  | `FileSystem.FileSystem`, `Path.Path`, `Crypto.Crypto`, `ExtensionSourceRootsPort`, `PackagedExtensionTemplatesPort`                                                                       | packaged builtin templates and app-owned live builtin source root                                                                                                                                                                                                                                                                                                                                                                                                                                                                           | yes                                                                                                                                                                                                                                                        | no              | no                                                                                                                                                                                  | full-batch preflight, packaged template missing, partial live root rejection, deterministic materialized/existing/app-native evidence, idempotent missing-only behavior, no source-checkout template path            |
...
`ToolDeclarationInput.requestInputVariant` is the exact core-owned `RequestInputVariant`. The
extension package remains the sole concrete native-tool declaration authority. It selects both the
`request_user_input` tool description and its `questions` property description from that variant:
`nonblocking` says defaults return immediately and later user answers arrive as follow-up messages;
`blocking` says the call waits for user answers or timeout defaults. Timeout enablement and duration
never enter declaration generation.

The public schema JSON builders require the same variant explicitly:

```ts
buildNativeToolSchemasJson(records, requestInputVariant);
buildNativeToolSchemaJsonForExtension(extension, requestInputVariant);
nativeToolDeclarationsForExtensions(records, requestInputVariant);
```

Thus generated native-tool schema bytes and their downstream fingerprints change when a loaded
Request User Input declaration changes mode, while timeout setting changes do not affect them.
...
@svvy/extensions/src/builtin/
  ...
  extension-managing/
    ...
    svvyx/
      commands.ts
  request-user-input/
    variants/nonblocking.md
    variants/blocking.md
  thread-orchestration/
...
- Request User Input mode guidance is the narrow exception: its exact nonblocking/blocking text is
  stored in the packaged variant Markdown assets above and selected by
  `getRequestUserInputVariantInstructions(requestInputVariant)`. Bun prompt composition consumes
  that extension-owned function and never carries duplicate hardcoded variant bodies.
...
The nonblocking declaration and packaged Markdown say that conservative defaults return immediately
and later answers are ordinary follow-up messages. The blocking declaration and packaged Markdown
say that execution waits for a user answer or configured timeout fallback. Only the mode selects
these assets and declaration descriptions; timeout enablement and duration remain runtime/state
policy and are never embedded in generated schema or instruction fingerprints.
...
| ExtensionCliRequirementProbePort              | `@svvy/extensions` package-local CLI probe primitive                                                                                        | in-process service  | `layer-acquired`           | app/bootstrap supplies one bounded probe implementation while composing `Extensions.layer`                                                                                      | app runtime shutdown/restart                                                                               | yes                                                                   | an interrupted probe returns no successful readiness evidence; no state, install, build, Shell command, or managed secret authority is owned by the port           | root-layer requirement, fake-port, closed-plan, and no-ambient-env tests               |
| ExtensionBuildProcessPort                     | `@svvy/extensions` package-local bounded build primitive                                                                                    | in-process service  | `layer-acquired`           | app/bootstrap supplies one env-free bounded process implementation while composing `Extensions.layer`                                                                           | app runtime shutdown/restart                                                                               | yes                                                                   | interruption returns no successful staged evidence; the port owns no current promotion, state, runtime command, approval, dependency-install, or secret authority  | root-layer requirement, fake-port, contained-plan, and failed-promotion tests          |
| Extension snapshot secret-values host port    | core-owned `ExtensionSnapshotSecretValuesPort` implemented by app/bootstrap and consumed only by runtime snapshot orchestration             | host secret adapter | `layer-acquired`           | app/bootstrap composes one redacting capture/restore adapter into `Runtime.layer`; extensions and state receive no raw secret values                                            | app runtime shutdown/restart closes the host adapter when it owns a closeable handle                       | yes within app runtime                                                | interruption cancels only the active capture/restore; raw values and material references never enter product state, generated packages, logs, or snapshot metadata | redaction, capture/restore idempotency, missing-secret, and no-state-persistence tests |
...
Builtin and user extension ids are canonical extension-owned addresses:
`<extension-id>#<role>` for singleton records and
`<extension-id>#<role>/<percent-encoded-relative-identity>` for contributor records. Decoding and
re-encoding must reproduce the exact address; separator aliases, empty segments, `.`, `..`,
backslashes, and traversal are rejected. Implemented roles are `minimal`, `instruction`, `script`,
`generated-instruction`, `svvyx-source`, `command-schema`, `native-tool-schema`, and
`typescript-api-declaration`. The three generated tooling roles resolve only beneath the canonical
current build root `builds/extensions/<extension-id>/current/`; their role fixes the filename to
`commands.json`, `native-tool-schema.json`, or `index.d.ts`, respectively. They never accept a path
or relative filename from the caller.
...
source edit session resolves builtin/user extension minimal instruction Markdown, loaded source
contributors, scripted contributor sources, read-only generated contributor outputs, editable
`svvyx` command source, and current-build read-only command schemas, native-tool schemas, and
TypeScript declarations, plus Workflows
source-library files for `workflow-agent`, `workflow-prompt`, `workflow-component`, and
`workflow-workflow`. Generated tooling source identities are display-only current-build records;
they are not aliases for workspace-CWD or shared generated-package files.
All
...
- Normal builtin and user extension source identities are fragment-qualified canonical addresses.
  Generated instruction outputs and generated `commands.json` records open only when their
  app-owned backing file exists, and saves fail with `read-only-source`. Generated
