...
included, and must warn on non-Markdown files in that directory when they look accidental. If a
future manifest adds explicit instruction ordering metadata, missing referenced files and duplicate
logical entries must be build errors, and this spec must be updated before that metadata becomes
authoritative.
...
Use case: add, remove, rename, or reorder source files under `instructions/full/` without inventing a
text-editing API.

These commands operate only on full loaded instruction source files. They do not edit file body
content. After creating or renaming a file, agents edit the returned path with native `apply_patch`.
...
All successful lifecycle commands:
...
Removing the last full instruction file is allowed only if the extension genuinely has no loaded
instruction prose. The build still validates that a loaded extension with no full instruction files
has either native tools, `svvyx` command guidance, or an intentionally empty full-instruction set.
