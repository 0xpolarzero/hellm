...
Instead, local extension file changes mark the extension `buildRequired: true`, the UI shows an
actionable Build control, and the Extension Managing instructions tell the agent to run `build`
after it finishes the edit batch.
...
manifest, source, or editable package file submits a runtime source-edit save request with a
    ...
    user/<extension-id>/
      ...
      instructions/
        full/
          ...
          030-domain-guide.mdx
        minimal.mdx
      scripts/
        generate-docs.ts
      source/
    builtin/<extension-id>/
      manifest.json
      instructions/
        full/
          010-overview.mdx
...
Workspace-local extensions are not part of the product surface. Extension files are app-owned files,
...
- generated instruction declarations in `manifest.json`
...
from the agent's point of view. They are regenerated by `build`.
...
type ExtensionManifestV1 = {
  ...
  workflowTaskAgentReferenceExportEnabled?: boolean;
...
- `native_tool` manifests are app-owned builtin implementation records and cannot be created through
  `svvyx extensions create`; builtin native-tool sources may still expose editable title,
  description, and instructions through local builtin source storage.
...
- `cliRequirements` declare required command providers inspected or used by the extension. `binary`
...
- `instructionFiles` stores loaded MDX source contributors under `instructions/full/` plus
  per-contributor bypass state. The UI and Extension Managing present these as loaded instruction
...
Every normal builtin or user extension has one editable minimal MDX instruction source, except the
...
Editable source contributors are MDX files under:

```text
instructions/full/*.mdx
```

Scripted contributors pair an editable TypeScript generator with read-only generated Markdown/plain
prompt output:
...
concatenating the non-bypassed loaded contributors with stable contributor-boundary headings or
equivalent internal separators after editable MDX and scripted generated output are compiled to plain
...
020-incur-cli-authoring.mdx
030-domain-guide.mdx
```

The ordering rule is deliberately visible in `inspect` output. Extension Managing may still expose
convenience commands and UI controls for add, remove, rename, and reorder. Reorder updates the
app-owned contributor order and may also rename files to preserve readable numeric prefixes. Those
lifecycle operations mark the extension `buildRequired: true`.

Individual loaded contributors may be bypassed through `manifest.json` or equivalent local builtin
source config. Bypassed contributors remain visible in inspect/UI, and scripted outputs are
...
SQLite-backed `@svvy/state` product storage includes persisted facts derived from extension
lifecycle and usage:

- extension source fingerprints and source-version facts
- generated-context binding facts, fingerprints, diagnostics, and read-model indexes
...
readiness interpretation remain owned by `@svvy/extensions`. `@svvy/state` stores committed product
facts; it does not own the extension registry or build implementation.

Extension Managing exposes editable file paths for every local source item an agent is expected
to change. Agents edit normal local files under `~/.config/svvy/extensions/sources/...`; they do not
write extension instructions, manifests, command source, generator scripts, or editable package state
through a database-specific command.

Builtin extensions have packaged defaults plus local editable source directories:
...
- builtin title, description, instructions, and optional editable extension source are editable by
  updating local source files under `sources/builtin/<id>/`
...
- reset restores only `svvy` usage/settings or builtin metadata sources, not the external file
- delete is unavailable

User extensions are ordinary local extension directories under the app-owned extension source root
`~/.config/svvy/extensions/sources/user/<id>/`:

- user extension source lives under that root
...
Generated extension files live under the app-owned generated root
`~/.config/svvy/extensions/generated/extensions/<id>/`. In the normal agent-facing contract this
directory contains generated TypeScript declarations when TypeScript API is enabled; generated
command help is obtained from the loaded `svvyx` namespace itself. Generated agent context
aggregates live in the disposable cache under `~/.config/svvy/extensions/generated/aggregates/`.
Current build output lives under `~/.config/svvy/extensions/builds/extensions/<id>/current/`;
`~/.config/svvy/extensions/builds/extensions/<id>/staging/<build-run-id>/` exists only while a build
is running and is atomically promoted over `current/` after a successful build. Generated and build
outputs are read-only to ordinary Shell, `apply_patch`, and `execute_typescript` paths; source edits
must go through editable source files and rebuild. There is no preserved build-history directory, no
user-facing build rollback, and no build-retention or pruning policy because only the current build
is kept.

Generated actor context is produced by the `@svvy/extensions` generated-context service and
persisted/indexed only through `@svvy/state` generated-context facts, fingerprints, diagnostics, and
surface bindings. Implementations may use package-private memoization such as the aggregate cache
below, but no aggregate cache path is product state, editable source, a snapshot payload, or an
agent-facing inspection target.

Generated agent context aggregates use a package-private lightweight cache:
...
- aggregate cache pruning is based only on cache mechanics and must not encode product semantics; the
  ...
  exceeded
- the aggregate SQLite file and blob directory are private, non-authoritative `@svvy/extensions`
  cache implementation details; they are not `@svvy/state` product state and not a source of
  historical generated-context truth. `@svvy/state` stores only binding facts, fingerprints,
  diagnostics, and read-model indexes that are needed after cache eviction or restart.

Generated declaration paths, current build output, `package/bun.lock`, `node_modules`, trash, and
snapshots are inspectable when useful, but they are not editable source. Aggregate cache paths remain
package-private implementation details and are not an agent-facing inspection surface.
...
`svvy` should not use git to implement extension revert. Local extension source files may live outside a
repository, and the product only needs local app-history reversibility. `svvy` records structured
change records for lifecycle commands and records patch/preimage data for local extension source files
...
| ---------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Agent `apply_patch` touching local extension source files                                | Revert the whole recorded change, not individual files.                                                                                                                                                                     |
...
- `svvy` records one reversible file-change card per successful `apply_patch` call that touches
  local extension source files.
- Source edits, instruction edits, and manifest/metadata edits are the same class: local source file
  changes.
...
This namespace is documented in generated agent context only when Extension Managing is loaded. The
stable `svvyx` dispatcher may still resolve built app-owned namespaces from Shell; loaded state is
not the shell-dispatch security boundary. It is separate from the native `list_extensions` tool:
...
| Code                            | Meaning                                                                                                             |
| ------------------------------- | ------------------------------------------------------------------------------------------------------------------- |
| `INSTRUCTIONS_NOT_EDITABLE`     | The target extension has no editable local instruction source for the requested instruction-file lifecycle command. |
...
| `INVALID_INSTRUCTION_FILENAME`  | A requested instruction basename is not an accepted MDX basename under `instructions/full/`.                        |
...
```json
{
  ...
  "extension": {
    ...
    "paths": {
      "sourceRoot": "/Users/example/.config/svvy/extensions/sources/builtin/github",
      "manifest": "/Users/example/.config/svvy/extensions/sources/builtin/github/manifest.json",
      "instructionsFull": [
        {
          "name": "010-github.mdx",
          "path": "/Users/example/.config/svvy/extensions/sources/builtin/github/instructions/full/010-github.mdx",
          "bypassed": false
        }
      ],
      "instructionsFullDir": "/Users/example/.config/svvy/extensions/sources/builtin/github/instructions/full",
      "instructionsMinimal": "/Users/example/.config/svvy/extensions/sources/builtin/github/instructions/minimal.mdx",
  ...
  "extension": {
    ...
    "paths": {
      ...
      "instructionsFull": [
        {
          "name": "010-linear.mdx",
          "path": "/Users/example/.config/svvy/extensions/sources/user/linear/instructions/full/010-linear.mdx",
          "bypassed": false
        }
      ],
      "instructionsFullDir": "/Users/example/.config/svvy/extensions/sources/user/linear/instructions/full",
      "instructionsMinimal": "/Users/example/.config/svvy/extensions/sources/user/linear/instructions/minimal.mdx",
      "externalInstructionFile": null,
    ...
    "state": {
      ...
      "issues": [
        ...
        {
          "code": "BUILD_REQUIRED",
          "message": "Linear has source changes that must be built before a replacement context can activate."
        }
      ]
    }
  }
...
- `manifest.json`
- `instructions/full/010-<extension-id>.mdx`
- `instructions/minimal.mdx`
- `source/` only when `--interface svvyx`

The generated initial loaded source contributor may be empty or contain only a short neutral MDX heading. It
...
These commands operate on loaded instruction contributors and their file-level config. Add, rename,
remove, and reorder operate only on editable MDX source contributors. Configure may target any
loaded contributor listed by `inspect`, including read-only generated `.generated.md` contributors.
They do not edit file body content. After creating or renaming a source file, agents edit the
returned path with native `apply_patch`.

All editable source file names are relative basenames under the extension's `instructions/full/`
directory. Accepted editable source names must:
...
- user extensions may use these commands when they have local instruction source storage
- builtin extensions may use these commands only through local builtin source files under
  `sources/builtin/<id>/`
- builtin packaged defaults must never be mutated directly
- removing or renaming a builtin default instruction file records local source metadata so reset can
  restore the packaged file set
- prompt-only, `svvyx`, and native-tool builtin extensions may all use these commands if they have
  editable local instruction sources
...
```bash
svvyx extensions instructions add <id> --name 020-domain-guide.mdx --json
```
...
| --------- | -------- | -------------------------------------------- |
| `<id>`    | yes      | Stable extension id.                         |
| `--name`  | yes      | New MDX basename under `instructions/full/`. |
| `--json`  | no       | Return machine-readable JSON.                |
...
```json
{
  ...
  "created": {
    ...
    "path": "/Users/example/.config/svvy/extensions/sources/user/linear/instructions/full/020-domain-guide.mdx"
  },
  "instructionsFullDir": "/Users/example/.config/svvy/extensions/sources/user/linear/instructions/full",
  "instructionsFull": [
    {
...
The created file should contain a minimal MDX heading or be empty. It must not contain generated
placeholder instruction prose that could accidentally be loaded as product guidance.

### `instructions rename`

```bash
svvyx extensions instructions rename <id> --from 020-domain-guide.mdx --to 030-domain-guide.mdx --json
```
...
| --------- | -------- | ------------------------------------------------- |
| `<id>`    | yes      | Stable extension id.                              |
| `--from`  | yes      | Existing MDX basename under `instructions/full/`. |
| `--to`    | yes      | New MDX basename under `instructions/full/`.      |
| `--json`  | no       | Return machine-readable JSON.                     |
...
```json
{
  ...
  "renamed": {
    ...
    "path": "/Users/example/.config/svvy/extensions/sources/user/linear/instructions/full/030-domain-guide.mdx"
  },
  "instructionsFullDir": "/Users/example/.config/svvy/extensions/sources/user/linear/instructions/full",
  "instructionsFull": [
    {
...
```bash
svvyx extensions instructions remove <id> --name 030-domain-guide.mdx --json
```
...
| --------- | -------- | ------------------------------------------------- |
| `<id>`    | yes      | Stable extension id.                              |
| `--name`  | yes      | Existing MDX basename under `instructions/full/`. |
| `--json`  | no       | Return machine-readable JSON.                     |
...
```json
{
  ...
  "instructionsFull": [
    ...
    {
      "name": "020-client.mdx",
      "path": "/Users/example/.config/svvy/extensions/sources/user/linear/instructions/full/020-client.mdx",
      "bypassed": false
    },
    {
      "name": "030-domain-guide.mdx",
...
For example, `030-client.mdx` in the second position becomes `020-client.mdx`, and
`domain-guide.mdx` in the third position becomes `030-domain-guide.mdx`.
...
| ------------ | -------- | ---------------------------------------------------------------- |
| `<id>`       | yes      | Stable extension id.                                             |
| `--file`     | yes      | Existing loaded contributor basename under `instructions/full/`. |
...
```json
{
  ...
  "instructionsFullDir": "/Users/example/.config/svvy/extensions/sources/builtin/smithers/instructions/full",
  "instructionsFull": [
    {
      "name": "010-smithers-core.generated.md",
      "path": "/Users/example/.config/svvy/extensions/sources/builtin/smithers/instructions/full/010-smithers-core.generated.md",
      "bypassed": false
    },
    {
      "name": "040-smithers-memory.generated.md",
      "path": "/Users/example/.config/svvy/extensions/sources/builtin/smithers/instructions/full/040-smithers-memory.generated.md",
      "bypassed": false
    }
  ],
  "buildRequired": true
...
context fingerprint changes. `agentContextImpact.affectedSurfaces` is DB/product-state-backed data
computed by runtime/state after the committed state change, not a preview supplied by the child
process, not duplicated extension metadata, and not data inferred from stdout.
...
For `--scope source`, reset restores builtin executable source such as `source/index.ts` from the
packaged default when the builtin extension has editable `svvyx` source. Generated outputs such as
`commands.json`, generated Markdown, and TypeScript declarations are not edited by reset; they are
rebuilt by the normal build path after source changes.
...
```json
{
  ...
  "result": {
    "resetFiles": [
      "/Users/example/.config/svvy/extensions/sources/builtin/github/instructions/full/010-github.mdx",
      "/Users/example/.config/svvy/extensions/sources/builtin/github/instructions/minimal.mdx"
    ],
    "buildRequired": true
  }
}
