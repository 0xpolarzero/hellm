...
Instead, local extension file changes mark the extension `buildRequired: true`, the UI shows a
...
Individual loaded contributors may be skipped through `manifest.json` or equivalent local builtin
source config. Skipped contributors remain visible in inspect/UI, and scripted outputs are
