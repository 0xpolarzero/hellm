...
- `svvy` does not expose `api.cx_*`, `extensions.cx.*`, or any generated cx TypeScript facade in
  v1.
...
TypeScript facade is registered.
...
For the shipped app, that script is the editable source-backed generator in the cx extension source
root. Packaged builtin templates only scaffold missing builtin source and reset builtin source to
defaults; runtime and extension build must not depend on a source-checkout path.
...
```text
bun scripts/generate-instructions.ts --output <absolute-output-path> --version <exact-version>
```
...
- mention `svvyx cx`, native `cx_*` tools, generated cx facades, or `prompt-only` in the generated
  Markdown
...
3. Run `svvyx extensions build cx --json`, which invokes source-root
   `scripts/generate-instructions.ts` for the new exact version.
...
- cx declares `scripts/generate-instructions.ts` as the generated instruction script.
- cx generated instruction declaration uses `versionCliRequirementId: "cx"`.
- `scripts/generate-instructions.ts` is invoked from the cx extension source root as
...
- Generated `execute_typescript` declarations do not include cx facades.
...
- cx is a prompt-only builtin extension.
- cx is loaded by default for all adopted actor kinds.
- cx loads the official upstream `cx skill` content from a versioned crates.io artifact.
- cx uses source-root `scripts/generate-instructions.ts` to generate
  `instructions/full/010-cx-skill.generated.md`.
- cx does not rewrite upstream `Read tool` or `Edit tool` wording in the generated skill file.
- cx keeps `svvy` product-boundary notes out of the generated upstream skill file.
- cx does not expose native `cx_*` tools.
- cx does not expose `svvyx cx`.
- cx does not expose generated cx TypeScript facades.
- cx does not expose an Incur wrapper to agents.
- cx does not create a custom editing or writing surface.
- cx does not own product navigation or product state.
- cx depends on declared CLI requirement resolution for missing binaries, not agent-facing
